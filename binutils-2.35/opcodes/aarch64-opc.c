  ...
  switch (opnd->type)
		  ...
		  && kind == AARCH64_MOD_UXTX)))
	{
      ...
      if (opnd->shifter.amount)
        ...
        { //In this branch, we know opnd->shifter.amount = 0 and kind may not be LSL
        if((!strcmp(opcode->name, "add") ||
           !strcmp(opcode->name,"adds") ||
           !strcmp(opcode->name,"sub") ||
           !strcmp(opcode->name,"subs") ||
           !strcmp(opcode->name,"cmp") ||
           !strcmp(opcode->name,"cmn")) && !strcmp(aarch64_operand_modifiers[kind].name,"lsl"))
            snprintf (buf, size, "%s",
              get_int_reg_name (opnd->reg.regno, opnd->qualifier, 0));
              else
