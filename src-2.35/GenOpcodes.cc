...
{
public:
  GenDefUseEntry(char *op_str) : op_str_(op_str)
  {
...
{
public:
  RegEntry(char *name, int num) : name_(strdup(name)), num_(num) {}
  char *name_;
  int num_;
};
...
void ReadSideEffects(const char *fname, MnemMap &mnem_map)
{
  ...
  while (!feof(f))
  {
    ...
    if (buff[0] == '#' || buff[0] == '\n')
      continue;
    // p指向buff从前向后找空格后面的内容，mnem和buff指向空格前面的内容（首尾空格已经去掉）
    ...
    while (p && *p && (p < end))
    {
      ...
      if (!strcasecmp(q, "all:"))
        mask = &e->reg_mask;
      else if (!strcasecmp(q, "addr8:"))
        mask = &e->reg_mask8;
      else if (!strcasecmp(q, "addr16:"))
        mask = &e->reg_mask16;
      else if (!strcasecmp(q, "addr32:"))
        mask = &e->reg_mask32;
      else if (!strcasecmp(q, "addr64:"))
        mask = &e->reg_mask64;
      else

          if (!strcasecmp(q, "flags:")) /* TODO(rhundt): refine */
        ;
      else if (!strcasecmp(q, "clear:")) /* TODO(rhundt): refine */
        ;
      else if (!strcasecmp(q, "undef:")) /* TODO(rhundt): refine */
        ;
      else

          if (!strcasecmp(q, "CF"))
        eflags = true;
      else if (!strcasecmp(q, "PF"))
        eflags = true;
      else if (!strcasecmp(q, "AF"))
        eflags = true;
      else if (!strcasecmp(q, "ZF"))
        eflags = true;
      else if (!strcasecmp(q, "SF"))
        eflags = true;
      else if (!strcasecmp(q, "TP"))
        eflags = true;
      else if (!strcasecmp(q, "IF"))
        eflags = true;
      else if (!strcasecmp(q, "DF"))
        eflags = true;
      else if (!strcasecmp(q, "OF"))
        eflags = true;
      else if (!strcasecmp(q, "IOPL"))
        eflags = true;
      else if (!strcasecmp(q, "NT"))
        eflags = true;
      else if (!strcasecmp(q, "RF"))
        eflags = true;
      else if (!strcasecmp(q, "VM"))
        eflags = true;
      else if (!strcasecmp(q, "AC"))
        eflags = true;
      else if (!strcasecmp(q, "VIF"))
        eflags = true;
      else if (!strcasecmp(q, "VIP"))
        eflags = true;
      else if (!strcasecmp(q, "ID"))
        eflags = true;
      else
          /*
      ...
      if (!strcasecmp(q, "ID")) e->flags.ID = true; else */
          ...
          if (!strcasecmp(q, "op0"))
        e->op_mask |= REG_OP0;
      else if (!strcasecmp(q, "src"))
        e->op_mask |= REG_OP0;
      else

          if (!strcasecmp(q, "op1"))
        e->op_mask |= REG_OP1;
      else if (!strcasecmp(q, "dest"))
        e->op_mask |= REG_OP1;
      else if (!strcasecmp(q, "op2"))
        e->op_mask |= REG_OP2;
      else if (!strcasecmp(q, "op3"))
        e->op_mask |= REG_OP3;
      else if (!strcasecmp(q, "op4"))
        e->op_mask |= REG_OP4;
      else if (!strcasecmp(q, "op5"))
        e->op_mask |= REG_OP5;
      else if (!strcasecmp(q, "exp"))
        e->op_mask |= USE_OP_ALL;
      else
      {
        // register
...
int main(int argc, char *const argv[])
{
  ...
  fprintf(def,
          ...
          "#define BNULL BitString(512, 8, 0x0ull, 0x0ull, 0x0ull, 0x0ull, 0x0ull, 0x0ull, 0x0ull, 0x0ull)\n"
          "#define BALL  BitString(512, 8, -1ull, -1ull, -1ull, -1ull, -1ull, -1ull, -1ull, -1ull)\n"
...
// 只找指令的name
    ...
    // 跳过重复的指令
    // 通过opc.tbl里的opcode通过use和def里的定义打印到gen-use.h和gen-def.h
    /* compare and emit */
    if (strcmp(name, lastname))
    {
      // 生成了opcode的name和enum定义和对应关系
      fprintf(out, "  OP_%s,\n", sanitized_name);
      ...
      else
      {
        ...
        // 标记在opcode-tbl中找到了对应的opcode
        e->found = true;
        if (e->op_mask & REG_OP0)
          fprintf(def, " | REG_OP0");
        if (e->op_mask & REG_OP1)
          fprintf(def, " | REG_OP1");
        if (e->op_mask & REG_OP2)
          fprintf(def, " | REG_OP2");
        if (e->op_mask & REG_OP3)
          fprintf(def, " | REG_OP3");
        if (e->op_mask & REG_OP4)
          fprintf(def, " | REG_OP4");
        if (e->op_mask & REG_OP5)
          fprintf(def, " | REG_OP5");
  ...
  // 检查maodef里的opcode是否都在opc-tbl里出现里
  if (emit_warnings)
