...
enum compressed_debug_section_type flag_compress_debug = COMPRESS_DEBUG_GABI_ZLIB;
#endif
...
parse_args(int *pargc, char ***pargv)
{
  ...
  longopts = (struct option *)xmalloc(sizeof(std_longopts) + md_longopts_size + sizeof(struct option));
...
//  Restore optind, otherwise can't get options when invoking gas twice.
  optind = 1;
  while (1)
  {
...
This program has absolutely no warranty.\n"));
    ...
    case 'g':
      ...
      if (old_argv[optind - 1][1] == 'g' && md_parse_option(optc, optarg))
        continue;
    ...
    case OPTION_GDWARF_CIE_VERSION:
      ...
      /* The available CIE versions are 1 (DWARF 2), 3 (DWARF 3), and 4
             (DWARF 4 and 5).  */
      if (flag_dwarf_cie_version < 1 || flag_dwarf_cie_version == 2 || flag_dwarf_cie_version > 4)
...
perform_an_assembly_pass(int argc, char **argv)
{
  ...
  bfd_set_section_flags(text_section,
                        applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_CODE | SEC_READONLY));
...
#define OBJ_DEFAULT_OUTPUT_FILE_NAME "a.out"
  ...
  if (argc > 1 && stat(out_file_name, &sob) == 0)
  {
    ...
    for (i = 1; i < argc; ++i)
    {
      ...
      if (stat(argv[i], &sib) == 0 && sib.st_ino == sob.st_ino
...
//  Disable write_object_file, otherwise error occurs when invoking gas twice
    if (!running_mao)
      write_object_file();
    ...
    if (n_errs == 0 && !running_mao)
      keep_it = 1;
    else if (flag_always_generate_output && !running_mao)
    {
