...
public:
  LabelEntry(const char *const name,
      ...
      : MaoEntry(line_number, line_verbatim, maounit),
        name_(strdup(name)),quotationFlag(f),from_assembly_(true) {}
...
public:
  enum Opcode
  {
    ...
    /*Following opcode is added to adjust Aarch64*/
    CFI_PERSONALITY_ID,
    CFI_FDE_DATA,
    CFI_INLINE_LSDA,
    CFI_VAL_OFFSET,
...
#define NUM_DATA_DIRECTIVES 8
  ...
  public:
    ...
    explicit Operand(symbolS *sym, int quotationFlag) : type(SYMBOL_QUOTATION) { data.sym = sym; }
    ...
    explicit Operand(const MaoStringPiece &str, const enum bfd_reloc_code_real reloc)
            : type(STRING_RELOC)
    {
       data.str = new std::string(str.data, str.length);
       data.expr_reloc.reloc = reloc;
  ...
  void setOperands(OperandVector v)  {  operands_=v; }
...
public:
  ...
  InstructionEntry(aarch64_instruction *instruction, enum flag_code flag_code,
                   unsigned int line_number, const char *line_verbatim,
                   MaoUnit *maounit);
  ~InstructionEntry();
...
//    return IsRegisterOperand(instruction(), op_index);
  ...
  {
//    return instruction_->types[op_index].bitfield.byte ||
//           (instruction_->types[op_index].bitfield.instance == Accum &&
//            instruction_->types[op_index].bitfield.byte);
  ...
  {
//    return instruction_->types[op_index].bitfield.word ||
//           (instruction_->types[op_index].bitfield.instance == Accum &&
//            instruction_->types[op_index].bitfield.word);
  ...
  {
//    return instruction_->types[op_index].bitfield.dword ||
//           (instruction_->types[op_index].bitfield.instance == Accum &&
//            instruction_->types[op_index].bitfield.dword);
  ...
  {
//    return instruction_->types[op_index].bitfield.fword ||
//           (instruction_->types[op_index].bitfield.instance == Accum &&
//            instruction_->types[op_index].bitfield.qword);
  ...
  {
//    return instruction_->types[op_index].bitfield.class_width == RegMMX;
  }
...
//    return instruction_->op[op_index].disps;
  ...
  // Returns a pointer to the binutils aarch64_instruction structure wrapped by this
  // instruction entry.
  aarch64_instruction *instruction() const { return instruction_; }
...
//    return instruction_->reloc[num];
  ...
  static bool IsMemOperand(const aarch64_instruction *instruction,
  ...
  static bool IsImmediateOperand(const aarch64_instruction *instruction,
                                 const unsigned int op_index);

  // Returns if the 'op_index'th operand of instruction is an immediate
  // operand and an actual integer. The form above also allows symbols
  // declared with .comm
  static bool IsImmediateIntOperand(const aarch64_instruction *instruction,
                                    const unsigned int op_index);
  // Returns if the 'op_index'th operand of instruction is a register operand.
  static bool IsRegisterOperand(const aarch64_instruction *instruction,
                                const unsigned int op_index);
...
private:
    aarch64_instruction *instruction_;
  MaoOpcode op_;
  ...
  aarch64_instruction *CreateInstructionCopy(aarch64_instruction *in_inst);
