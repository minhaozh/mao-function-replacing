      ...
      /* We copy this string: OK to alter later.  */)
{
  ...
  if ((symbolP = symbol_find(sym_name)) != 0)
  {
    ...
    if (LOCAL_SYMBOL_CHECK(symbolP))
    {
      ...
      if (locsym->lsy_section != undefined_section && (local_symbol_get_frag(locsym) != frag_now || locsym->lsy_section != now_seg || locsym->lsy_value != frag_now_fix()))
      {
    ...
    else if (!(S_IS_DEFINED(symbolP) || symbol_equated_p(symbolP)) || S_IS_COMMON(symbolP) || S_IS_VOLATILE(symbolP))
    {
      ...
      else
      {
        ...
        if (((!S_IS_DEBUG(symbolP) && (!S_IS_DEFINED(symbolP) || S_IS_COMMON(symbolP)) && S_IS_EXTERNAL(symbolP)) || S_GET_SEGMENT(symbolP) == bss_section) && (now_seg == data_section || now_seg == bss_section || now_seg == S_GET_SEGMENT(symbolP)))
        {
          ...
          if (now_seg != data_section)
          {
            /* New .comm for prev .comm symbol.
			 ...
			 this symbol.  */
            if (S_GET_VALUE(symbolP) < ((unsigned)frag_now_fix()))
            {
              S_SET_VALUE(symbolP, (valueT)frag_now_fix());
            }
          }
    ...
    else
    {
      ...
      if (!(frag_now == symbolP->sy_frag && S_GET_VALUE(symbolP) == frag_now_fix() && S_GET_SEGMENT(symbolP) == now_seg))
      {
        as_bad(_("symbol `%s' is already defined"), sym_name);
...
{
  if (s != NULL && s->sy_flags.sy_local_symbol && local_symbol_converted_p((struct local_symbol *)s))
    s = local_symbol_get_real_symbol((struct local_symbol *)s);
  return s;
}
...
symbol_clone_if_forward_ref(symbolS *symbolP, int is_forward)
{
  if (symbolP && !LOCAL_SYMBOL_CHECK(symbolP))
  {
    ...
    if ((symbolP->bsym->section == expr_section || symbolP->sy_flags.sy_forward_ref) && !symbolP->sy_flags.sy_resolving)
    {
    ...
    if (symbolP->sy_flags.sy_forward_ref || add_symbol != orig_add_symbol || op_symbol != orig_op_symbol)
    {
...
int symbol_on_chain(symbolS *s, symbolS *rootPP, symbolS *lastPP)
{
  return (!LOCAL_SYMBOL_CHECK(s) && ((s->sy_next != s && s->sy_next != NULL && s->sy_next->sy_previous == s) || s == lastPP) && ((s->sy_previous != s && s->sy_previous != NULL && s->sy_previous->sy_next == s) || s == rootPP));
}
...
use_complex_relocs_for(symbolS *symp)
{
  ...
  case O_logical_and:
  case O_logical_or:
    if ((S_IS_COMMON(symp->sy_value.X_op_symbol) || S_IS_LOCAL(symp->sy_value.X_op_symbol)) && S_IS_DEFINED(symp->sy_value.X_op_symbol) && S_GET_SEGMENT(symp->sy_value.X_op_symbol) != expr_section)
    {
    case O_symbol:
    case O_symbol_rva:
    case O_uminus:
    case O_bit_not:
    case O_logical_not:
      if ((S_IS_COMMON(symp->sy_value.X_add_symbol) || S_IS_LOCAL(symp->sy_value.X_add_symbol)) && S_IS_DEFINED(symp->sy_value.X_add_symbol) && S_GET_SEGMENT(symp->sy_value.X_add_symbol) != expr_section)
        return 0;
    }
    break;
...
#ifdef OBJ_COMPLEX_RELC
  else if (final_seg == expr_section && use_complex_relocs_for(symp))
  {
  ...
  else
  {
    ...
    do_symbol:
      if (S_IS_WEAKREFR(symp))
      {
        ...
        {
          gas_assert(add_symbol->sy_value.X_op == O_symbol && add_symbol->sy_value.X_add_number == 0);
      ...
      if (finalize_syms && !LOCAL_SYMBOL_CHECK(add_symbol) && add_symbol->sy_flags.sy_resolving)
        break;
      ...
      /* If we have equated this symbol to an undefined or common
	     ...
	     is equated.  */
      if (seg_left == undefined_section || bfd_is_com_section(seg_left)
#if defined(OBJ_COFF) && defined(TE_PE)
          || S_IS_WEAK(add_symbol)
#endif
          || (finalize_syms && ((final_seg == expr_section && seg_left != expr_section && seg_left != absolute_section) || symbol_shadow_p(symp))))
      {
        if (finalize_syms)
        {
          symp->sy_value.X_op = O_symbol;
    ...
    case O_logical_not:
      ...
      if (op != O_logical_not && seg_left != absolute_section && finalize_syms)
    ...
    case O_logical_or:
      ...
      if (!(seg_left == absolute_section && seg_right == absolute_section) && !(op == O_eq || op == O_ne) && !((op == O_subtract || op == O_lt || op == O_le || op == O_ge || op == O_gt) && seg_left == seg_right && (seg_left != undefined_section || add_symbol == op_symbol)))
      {
      ...
      if (move_seg_ok && (final_seg == expr_section || final_seg == undefined_section))
        final_seg = absolute_section;
      ...
      case O_eq:
      case O_ne:
        left = (left == right && seg_left == seg_right && (seg_left != undefined_section || add_symbol == op_symbol)
                    ? ~(offsetT)0
                    : 0);
      ...
      resolved = (symbol_resolved_p(add_symbol) && symbol_resolved_p(op_symbol));
      break;
...
S_GET_VALUE(symbolS *s)
{
  ...
  {
    if (!s->sy_flags.sy_resolved || s->sy_value.X_op != O_symbol || (S_IS_DEFINED(s) && !S_IS_COMMON(s)))
...
#define COPIED_SYMFLAGS (BSF_FUNCTION | BSF_OBJECT | BSF_GNU_INDIRECT_FUNCTION)
...
int S_FORCE_RELOC(symbolS *s, int strict)
{
  ...
  {
    if ((strict && ((s->bsym->flags & BSF_WEAK) != 0 || (EXTERN_FORCE_RELOC && (s->bsym->flags & BSF_GLOBAL) != 0))) || (s->bsym->flags & BSF_GNU_INDIRECT_FUNCTION) != 0)
      return TRUE;
    sec = s->bsym->section;
  }
...
int S_IS_LOCAL(symbolS *s)
{
  ...
  if (flag_strip_local_absolute
	 ...
	 the source file even when the object file is stripped.  */
      && (flags & (BSF_GLOBAL | BSF_FILE)) == 0 && bfd_asymbol_section(s->bsym) == absolute_section)
    return 1;
  ...
  /* Dollar local labels are not enabled in gas, and MAO only has a
     ...
     be updated. */
  name = S_GET_NAME(s);
  return (name != NULL && !S_IS_DEBUG(s) && (strchr(name, DOLLAR_LABEL_CHAR)
#if FAKE_LABEL_CHAR != DOLLAR_LABEL_CHAR
                                             || strchr(name, FAKE_LABEL_CHAR)
#endif
                                             || TC_LABEL_IS_LOCAL(name) || is_local_label(name) || (!flag_keep_locals && (bfd_is_local_label(stdoutput, s->bsym) || (flag_mri && name[0] == '?' && name[1] == '?')))));
}
...
{
  if (LOCAL_SYMBOL_CHECK(s))
    return (local_symbol_get_frag((struct local_symbol *)s) == &predefined_address_frag);
...
void S_SET_THREAD_LOCAL(symbolS *s)
{
  if (LOCAL_SYMBOL_CHECK(s))
    s = local_symbol_convert((struct local_symbol *)s);
  if (bfd_is_com_section(s->bsym->section) && (s->bsym->flags & BSF_THREAD_LOCAL) != 0)
    return;
  s->bsym->flags |= BSF_THREAD_LOCAL;
  if ((s->bsym->flags & BSF_FUNCTION) != 0)
    as_bad(_("Accessing function `%s' as thread-local object"),
           S_GET_NAME(s));
  else if (!bfd_is_und_section(s->bsym->section) && (s->bsym->section->flags & SEC_THREAD_LOCAL) == 0)
...
int symbol_equated_reloc_p(symbolS *s)
{
  ...
  return (s->sy_value.X_op == O_symbol
          ...
          && !S_IS_WEAK(s)
#endif
          && ((s->sy_flags.sy_resolved && s->sy_value.X_op_symbol != NULL) || !S_IS_DEFINED(s) || S_IS_COMMON(s)));
}
...
void symbol_set_obj(symbolS *s, OBJ_SYMFIELD_TYPE *o)
{
  if (LOCAL_SYMBOL_CHECK(s))
    s = local_symbol_convert((struct local_symbol *)s);
  s->sy_obj = *o;
...
void symbol_set_tc(symbolS *s, TC_SYMFIELD_TYPE *o)
{
  if (LOCAL_SYMBOL_CHECK(s))
    s = local_symbol_convert((struct local_symbol *)s);
  s->sy_tc = *o;
...
void print_symbol_value_1(FILE *file, symbolS *sym)
{
  ...
  if (LOCAL_SYMBOL_CHECK(sym))
  {
    ...
    if (local_symbol_get_frag(locsym) != &zero_address_frag && local_symbol_get_frag(locsym) != NULL)
    {
      fprintf(file, " frag ");
  ...
  else if (indent_level < max_indent_level && S_GET_SEGMENT(sym) != undefined_section)
  {
    indent_level++;
    fprintf(file, "\n%*s<", indent_level * 4, "");
    if (LOCAL_SYMBOL_CHECK(sym))
...
symbol_relc_make_sym(symbolS *sym)
{
  ...
  if (S_GET_SEGMENT(sym) == expr_section || S_GET_SEGMENT(sym) == absolute_section)
