...
static const reg_entry bad_reg = {"<bad>", OPERAND_TYPE_NONE, 0, 0, {Dw2Inval, Dw2Inval}};
...
#if (defined(TE_I386AIX) || ((defined(OBJ_ELF) || defined(OBJ_MAYBE_ELF)) && !defined(TE_GNU) && !defined(TE_LINUX) && !defined(TE_FreeBSD) && !defined(TE_DragonFly) && !defined(TE_NetBSD)))
...
#if ((defined(OBJ_MAYBE_COFF) && defined(OBJ_MAYBE_AOUT)) || defined(OBJ_ELF) || defined(OBJ_MAYBE_ELF) || defined(TE_PE) || defined(TE_PEP) || defined(OBJ_MACH_O))
...
static int generate_relax_relocations = DEFAULT_GENERATE_X86_RELAX_RELOCATIONS;
...
static unsigned int align_branch = (align_branch_jcc_bit | align_branch_fused_bit | align_branch_jmp_bit);
...
void i386_generate_nops(fragS *fragP, char *where, offsetT count, int limit)
{
	...
	case rs_machine_dependent:
		...
		if (TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == BRANCH_PADDING || TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == FUSED_JCC_PADDING)
			break;
		/* Fall through.  */
	default:
		return;
...
operand_type_set(union i386_operand_type *x, unsigned int v)
{
	...
	x->bitfield.class_width = ClassNone;
...
{
	return !((flag_code == CODE_64BIT && f.bitfield.cpuno64) || (flag_code != CODE_64BIT && f.bitfield.cpu64));
}
	...
	else
	{
		...
		if (!cpu_flags_all_zero(&cpu))
		{
			if (x.bitfield.cpuavx)
			{
				...
				if (cpu.bitfield.cpuavx && (!t->opcode_modifier.sse2avx || (sse2avx && !i.prefix[DATA_PREFIX])) && (!x.bitfield.cpuaes || cpu.bitfield.cpuaes) && (!x.bitfield.cpugfni || cpu.bitfield.cpugfni) && (!x.bitfield.cpupclmul || cpu.bitfield.cpupclmul))
					match |= CPU_FLAGS_ARCH_MATCH;
			}
			else if (x.bitfield.cpuavx512f)
			{
				/* We need to check a few extra flags with AVX512F.  */
				if (cpu.bitfield.cpuavx512f && (!x.bitfield.cpugfni || cpu.bitfield.cpugfni) && (!x.bitfield.cpuvaes || cpu.bitfield.cpuvaes) && (!x.bitfield.cpuvpclmulqdq || cpu.bitfield.cpuvpclmulqdq))
					match |= CPU_FLAGS_ARCH_MATCH;
			}
			else
				match |= CPU_FLAGS_ARCH_MATCH;
...
{
	if (x.bitfield.class_width != y.bitfield.class_width)
		x.bitfield.class_width = ClassNone;
...
{
	gas_assert(y.bitfield.class_width == ClassNone);
...
{
	gas_assert(x.bitfield.class_width == ClassNone ||
			   y.bitfield.class_width == ClassNone ||
			   x.bitfield.class_width == y.bitfield.class_width);
...
match_simd_size(const insn_template *t, unsigned int wanted,
				unsigned int given)
{
	return !((i.types[given].bitfield.xmmword && !t->operand_types[wanted].bitfield.xmmword) || (i.types[given].bitfield.ymmword && !t->operand_types[wanted].bitfield.ymmword) || (i.types[given].bitfield.zmmword && !t->operand_types[wanted].bitfield.zmmword));
}
...
match_mem_size(const insn_template *t, unsigned int wanted,
			   unsigned int given)
{
	return (match_operand_size(t, wanted, given) && !((i.types[given].bitfield.unspecified && !i.broadcast && !t->operand_types[wanted].bitfield.unspecified) || (i.types[given].bitfield.fword && !t->operand_types[wanted].bitfield.fword)
		  ...
		  down-conversion vpmov*.  */
													  || ((t->operand_types[wanted].bitfield.class_width == RegSIMD && t->operand_types[wanted].bitfield.byte + t->operand_types[wanted].bitfield.word + t->operand_types[wanted].bitfield.dword + t->operand_types[wanted].bitfield.qword > !!t->opcode_modifier.broadcast)
															  ? (i.types[given].bitfield.xmmword || i.types[given].bitfield.ymmword || i.types[given].bitfield.zmmword)
...
operand_size_match(const insn_template *t)
{
	...
	if (t->opcode_modifier.jump && t->opcode_modifier.jump != JUMP_ABSOLUTE)
		return match;

	/* Check memory and accumulator operand size.  */
	for (j = 0; j < i.operands; j++)
	{
		if (i.types[j].bitfield.class_width != Reg && i.types[j].bitfield.class_width != RegSIMD && t->opcode_modifier.anysize)
			continue;

		if (t->operand_types[j].bitfield.class_width == Reg && !match_operand_size(t, j, j))
		{
			match = 0;
			break;
		}

		if (t->operand_types[j].bitfield.class_width == RegSIMD && !match_simd_size(t, j, j))
		{
			match = 0;
			break;
		}

		if (t->operand_types[j].bitfield.instance == Accum && (!match_operand_size(t, j, j) || !match_simd_size(t, j, j)))
		{
			match = 0;
			break;
		}
	...
	for (j = 0; j < i.operands; j++)
	{
		...
		if (t->operand_types[j].bitfield.class_width == Reg && !match_operand_size(t, j, given))
			goto mismatch;

		if (t->operand_types[j].bitfield.class_width == RegSIMD && !match_simd_size(t, j, given))
			goto mismatch;

		if (t->operand_types[j].bitfield.instance == Accum && (!match_operand_size(t, j, given) || !match_simd_size(t, j, given)))
			goto mismatch;
...
{
	if (g0.bitfield.class_width != Reg && g0.bitfield.class_width != RegSIMD && (!operand_type_check(g0, anymem) || g0.bitfield.unspecified || (t0.bitfield.class_width != Reg && t0.bitfield.class_width != RegSIMD)))
		return 1;

	if (g1.bitfield.class_width != Reg && g1.bitfield.class_width != RegSIMD && (!operand_type_check(g1, anymem) || g1.bitfield.unspecified || (t1.bitfield.class_width != Reg && t1.bitfield.class_width != RegSIMD)))
		return 1;

	if (g0.bitfield.byte == g1.bitfield.byte && g0.bitfield.word == g1.bitfield.word && g0.bitfield.dword == g1.bitfield.dword && g0.bitfield.qword == g1.bitfield.qword && g0.bitfield.xmmword == g1.bitfield.xmmword && g0.bitfield.ymmword == g1.bitfield.ymmword && g0.bitfield.zmmword == g1.bitfield.zmmword)
		return 1;

	if (!(t0.bitfield.byte & t1.bitfield.byte) && !(t0.bitfield.word & t1.bitfield.word) && !(t0.bitfield.dword & t1.bitfield.dword) && !(t0.bitfield.qword & t1.bitfield.qword) && !(t0.bitfield.xmmword & t1.bitfield.xmmword) && !(t0.bitfield.ymmword & t1.bitfield.ymmword) && !(t0.bitfield.zmmword & t1.bitfield.zmmword))
		return 1;
...
add_prefix(unsigned int prefix)
{
	...
	{
		if ((i.prefix[REX_PREFIX] & prefix & REX_W) || (i.prefix[REX_PREFIX] & prefix & REX_R) || (i.prefix[REX_PREFIX] & prefix & REX_X) || (i.prefix[REX_PREFIX] & prefix & REX_B))
			ret = PREFIX_EXIST;
		q = REX_PREFIX;
	}
	else
						  ...
						  i386_cpu_flags new_flag ATTRIBUTE_UNUSED)
{
	...
	if (get_elf_backend_data(stdoutput)->elf_machine_code != EM_IAMCU || new_flag.bitfield.cpuiamcu)
		return;
...
i386_mach(void)
{
	...
	else if (!strcmp(default_arch, "i386") || !strcmp(default_arch, "iamcu"))
	{
...
pi(const char *line, i386_insn *x)
{
	...
	for (j = 0; j < x->operands; j++)
	{
		...
		pt(x->types[j]);
		fprintf(stdout, "\n");
		if (x->types[j].bitfield.class_width == Reg || x->types[j].bitfield.class_width == RegMMX || x->types[j].bitfield.class_width == RegSIMD || x->types[j].bitfield.class_width == RegMask || x->types[j].bitfield.class_width == SReg || x->types[j].bitfield.class_width == RegCR || x->types[j].bitfield.class_width == RegDR || x->types[j].bitfield.class_width == RegTR || x->types[j].bitfield.class_width == RegBND)
	  ...
	  bfd_reloc_code_real_type other)
{
	if (other != NO_RELOC)
	{
		...
		else if ((rel->complain_on_overflow == complain_overflow_signed && !sign) || (rel->complain_on_overflow == complain_overflow_unsigned && sign > 0))
...
build_vex_prefix(const insn_template *t)
{
	...
	if (i.reg_operands > 1 && i.vec_encoding != vex_encoding_vex3 && i.dir_encoding == dir_encoding_default && i.operands == i.reg_operands && operand_type_equal(&i.types[0], &i.types[i.operands - 1]) && i.tm.opcode_modifier.vexopcode == VEX0F && (i.tm.opcode_modifier.load || i.tm.opcode_modifier.d) && i.rex == REX_B)
	{
	...
	if (i.reg_operands >= 3 && i.vec_encoding != vex_encoding_vex3 && i.reg_operands == i.operands - i.imm_operands && i.tm.opcode_modifier.vex && i.tm.opcode_modifier.commutative && (i.tm.opcode_modifier.sse2avx || optimize > 1) && i.rex == REX_B && i.vex.register_specifier && !(i.vex.register_specifier->reg_flags & RegRex))
	{
	...
	else
	{
	 ...
	 operand.  */
		...
		for (op = t->operands; op--;)
			if (t->operand_types[op].bitfield.xmmword && t->operand_types[op].bitfield.ymmword && i.types[op].bitfield.ymmword)
			{
				vector_length = 1;
				break;
			}
	...
	if (w == 0 && i.vec_encoding != vex_encoding_vex3 && i.tm.opcode_modifier.vexopcode == VEX0F && (i.rex & (REX_W | REX_X | REX_B)) == 0)
	{
		/* 2-byte VEX prefix.  */
		unsigned int r;
...
{
	return t->opcode_modifier.evex || t->opcode_modifier.disp8memshift || t->opcode_modifier.broadcast || t->opcode_modifier.masking || t->opcode_modifier.sae;
}

static INLINE bfd_boolean
is_any_vex_encoding(const insn_template *t)
{
	return t->opcode_modifier.vex || t->opcode_modifier.vexopcode || is_evex_encoding(t);
}
...
build_evex_prefix(void)
{
	...
	if (!i.rounding)
	{
		...
		if (!i.tm.opcode_modifier.evex || i.tm.opcode_modifier.evex == EVEXDYN)
		{
			unsigned int op;
	     ...
	     operand.  */
			for (op = i.operands; op--;)
				if (i.tm.operand_types[op].bitfield.xmmword + i.tm.operand_types[op].bitfield.ymmword + i.tm.operand_types[op].bitfield.zmmword > 1)
				{
...
process_immext(void)
{
	...
	gas_assert(i.imm_operands <= 1 && (i.operands <= 2 || (is_any_vex_encoding(&i.tm) && i.operands <= 4)));
...
optimize_encoding(void)
{
	...
	if (optimize_for_space && !is_any_vex_encoding(&i.tm) && i.reg_operands == 1 && i.imm_operands == 1 && !i.types[1].bitfield.byte && i.op[0].imms->X_op == O_constant && fits_in_imm7(i.op[0].imms->X_add_number) && (i.tm.base_opcode == 0xa8 || (i.tm.base_opcode == 0xf6 && i.tm.extension_opcode == 0x0)))
	{
		/* Optimize: -Os:
	...
	else if (flag_code == CODE_64BIT && !is_any_vex_encoding(&i.tm) && ((i.types[1].bitfield.qword && i.reg_operands == 1 && i.imm_operands == 1 && i.op[0].imms->X_op == O_constant && ((i.tm.base_opcode == 0xb8 && i.tm.extension_opcode == None && fits_in_unsigned_long(i.op[0].imms->X_add_number)) || (fits_in_imm31(i.op[0].imms->X_add_number) && ((i.tm.base_opcode == 0x24 || i.tm.base_opcode == 0xa8) || (i.tm.base_opcode == 0x80 && i.tm.extension_opcode == 0x4) || ((i.tm.base_opcode == 0xf6 || (i.tm.base_opcode | 1) == 0xc7) && i.tm.extension_opcode == 0x0))) || (fits_in_imm7(i.op[0].imms->X_add_number) && i.tm.base_opcode == 0x83 && i.tm.extension_opcode == 0x4))) || (i.types[0].bitfield.qword && ((i.reg_operands == 2 && i.op[0].regs == i.op[1].regs && (i.tm.base_opcode == 0x30 || i.tm.base_opcode == 0x28)) || (i.reg_operands == 1 && i.operands == 1 && i.tm.base_opcode == 0x30)))))
	{
		/* Optimize: -O:
	...
	else if (optimize > 1 && !optimize_for_space && !is_any_vex_encoding(&i.tm) && i.reg_operands == 2 && i.op[0].regs == i.op[1].regs && ((i.tm.base_opcode & ~(Opcode_D | 1)) == 0x8 || (i.tm.base_opcode & ~(Opcode_D | 1)) == 0x20) && (flag_code != CODE_64BIT || !i.types[0].bitfield.dword))
	{
		/* Optimize: -O2:
	...
	else if (i.reg_operands == 3 && i.op[0].regs == i.op[1].regs && !i.types[2].bitfield.xmmword && (i.tm.opcode_modifier.vex || ((!i.mask || i.mask->zeroing) && !i.rounding && is_evex_encoding(&i.tm) && (i.vec_encoding != vex_encoding_evex || cpu_arch_isa_flags.bitfield.cpuavx512vl || i.tm.cpu_flags.bitfield.cpuavx512vl || (i.tm.operand_types[2].bitfield.zmmword && i.types[2].bitfield.ymmword)))) && ((i.tm.base_opcode == 0x55 || i.tm.base_opcode == 0x6655 || i.tm.base_opcode == 0x66df || i.tm.base_opcode == 0x57 || i.tm.base_opcode == 0x6657 || i.tm.base_opcode == 0x66ef || i.tm.base_opcode == 0x66f8 || i.tm.base_opcode == 0x66f9 || i.tm.base_opcode == 0x66fa || i.tm.base_opcode == 0x66fb || i.tm.base_opcode == 0x42 || i.tm.base_opcode == 0x6642 || i.tm.base_opcode == 0x47 || i.tm.base_opcode == 0x6647) && i.tm.extension_opcode == None))
	{
		/* Optimize: -O1:
       ...
       */
		...
		else if (i.tm.operand_types[0].bitfield.class_width == RegMask)
		{
	...
	else if (i.vec_encoding != vex_encoding_evex && !i.types[0].bitfield.zmmword && !i.types[1].bitfield.zmmword && !i.mask && !i.broadcast && is_evex_encoding(&i.tm) && ((i.tm.base_opcode & ~Opcode_SIMD_IntD) == 0x666f || (i.tm.base_opcode & ~Opcode_SIMD_IntD) == 0xf36f || (i.tm.base_opcode & ~Opcode_SIMD_IntD) == 0xf26f || (i.tm.base_opcode & ~4) == 0x66db || (i.tm.base_opcode & ~4) == 0x66eb) && i.tm.extension_opcode == None)
	{
		/* Optimize: -O1:
       ...
       */
		for (j = 0; j < i.operands; j++)
			if (operand_type_check(i.types[j], disp) && i.op[j].disps->X_op == O_constant)
			{
		...
		i.tm.opcode_modifier.vex = i.types[0].bitfield.ymmword ? VEX256 : VEX128;
		i.tm.opcode_modifier.vexw = VEXW0;
		...
		i.memshift = 0;
		if (j < i.operands)
			i.types[j].bitfield.disp8 = fits_in_disp8(i.op[j].disps->X_add_number);
	}
}
...
load_insn_p(void)
{
	...
	if (!any_vex_p)
	{
	 ...
	 bndstx, bndldx, prefetchwt1, clflushopt, clwb, cldemote.  */
		...
		/* pop, popf, popa.   */
		if (strcmp(i.tm.name, "pop") == 0 || i.tm.base_opcode == 0x9d || i.tm.base_opcode == 0x61)
			return 1;
		...
		/* outs, xlatb.  */
		if (base_opcode == 0x6f || i.tm.base_opcode == 0xd7)
			return 1;
	...
	{
		/* vldmxcsr.  */
		if (i.tm.base_opcode == 0xae && i.tm.opcode_modifier.vex && i.tm.opcode_modifier.vexopcode == VEX0F && i.tm.extension_opcode == 2)
			return 1;
	}
	else
	{
		/* test, not, neg, mul, imul, div, idiv.  */
		if ((i.tm.base_opcode == 0xf6 || i.tm.base_opcode == 0xf7) && i.tm.extension_opcode != 1)
			return 1;
		...
		/* bt, bts, btr, btc.  */
		if (i.tm.base_opcode == 0xfba && (i.tm.extension_opcode >= 4 && i.tm.extension_opcode <= 7))
			return 1;

		/* rol, ror, rcl, rcr, shl/sal, shr, sar. */
		if ((base_opcode == 0xc1 || (i.tm.base_opcode >= 0xd0 && i.tm.base_opcode <= 0xd3)) && i.tm.extension_opcode != 6)
			return 1;

		/* cmpxchg8b, cmpxchg16b, xrstors.  */
		if (i.tm.base_opcode == 0xfc7 && (i.tm.extension_opcode == 1 || i.tm.extension_opcode == 3))
			return 1;

		/* fxrstor, ldmxcsr, xrstor.  */
		if (i.tm.base_opcode == 0xfae && (i.tm.extension_opcode == 1 || i.tm.extension_opcode == 2 || i.tm.extension_opcode == 5))
			return 1;

		/* lgdt, lidt, lmsw.  */
		if (i.tm.base_opcode == 0xf01 && (i.tm.extension_opcode == 2 || i.tm.extension_opcode == 3 || i.tm.extension_opcode == 6))
			return 1;

		/* vmptrld */
		if (i.tm.base_opcode == 0xfc7 && i.tm.extension_opcode == 6)
			return 1;
		...
		if (i.tm.base_opcode >= 0xd8 && i.tm.base_opcode <= 0xdf)
		{
			...
			if (i.tm.base_opcode == 0xd9 && (i.tm.extension_opcode == 2 || i.tm.extension_opcode == 3 || i.tm.extension_opcode == 6 || i.tm.extension_opcode == 7))
				return 0;

			/* Skip fisttp, fist, fistp, fstp.  */
			if (i.tm.base_opcode == 0xdb && (i.tm.extension_opcode == 1 || i.tm.extension_opcode == 2 || i.tm.extension_opcode == 3 || i.tm.extension_opcode == 7))
				return 0;

			/* Skip fisttp, fst, fstp, fsave, fstsw.  */
			if (i.tm.base_opcode == 0xdd && (i.tm.extension_opcode == 1 || i.tm.extension_opcode == 2 || i.tm.extension_opcode == 3 || i.tm.extension_opcode == 6 || i.tm.extension_opcode == 7))
				return 0;

			/* Skip fisttp, fist, fistp, fbstp, fistp.  */
			if (i.tm.base_opcode == 0xdf && (i.tm.extension_opcode == 1 || i.tm.extension_opcode == 2 || i.tm.extension_opcode == 3 || i.tm.extension_opcode == 6 || i.tm.extension_opcode == 7))
				return 0;
	...
	if ((i.tm.opcode_modifier.immext || i.tm.opcode_modifier.vexsources == VEX3SOURCES) && i.types[dest].bitfield.imm8)
		dest--;

	/* add, or, adc, sbb, and, sub, xor, cmp, test, xchg, xadd  */
	if (!any_vex_p && (base_opcode == 0x1 || base_opcode == 0x9 || base_opcode == 0x11 || base_opcode == 0x19 || base_opcode == 0x21 || base_opcode == 0x29 || base_opcode == 0x31 || base_opcode == 0x39 || (i.tm.base_opcode >= 0x84 && i.tm.base_opcode <= 0x87) || base_opcode == 0xfc1))
		return 1;

	/* Check for load instruction.  */
	return (i.types[dest].bitfield.class_width != ClassNone || i.types[dest].bitfield.instance == Accum);
}
...
insert_lfence_after(void)
{
	if (lfence_after_load && load_insn_p())
	{
	 ...
	 of the secret.  */
		if (((i.tm.base_opcode | 0x1) == 0xa7 || (i.tm.base_opcode | 0x1) == 0xaf) && i.prefix[REP_PREFIX])
		{
...
insert_lfence_before(void)
{
	...
	if (i.tm.base_opcode == 0xff && (i.tm.extension_opcode == 2 || i.tm.extension_opcode == 4))
	{
		...
		if (i.reg_operands == 1)
		{
	     ...
	     -mlfence-after-load=yes.  */
			if (lfence_after_load || lfence_before_indirect_branch == lfence_branch_memory)
				return;
		}
		else if (i.mem_operands == 1 && lfence_before_indirect_branch != lfence_branch_register)
		{
	...
	if (lfence_before_ret != lfence_before_ret_none && (i.tm.base_opcode == 0xc2 || i.tm.base_opcode == 0xc3))
	{
		if (last_insn.kind != last_insn_other && last_insn.seg == now_seg)
		{
			as_warn_where(last_insn.file, last_insn.line,
...
// 继续解析操作数
	...
	/* All Intel opcodes have reversed operands except for "bound", "enter",
     ...
     mode.  */
	if (intel_syntax && i.operands > 1 && (strcmp(mnemonic, "bound") != 0) && (strcmp(mnemonic, "invlpga") != 0) && (strncmp(mnemonic, "monitor", 7) != 0) && (strncmp(mnemonic, "mwait", 5) != 0) && (strcmp(mnemonic, "tpause") != 0) && (strcmp(mnemonic, "umwait") != 0) && !(operand_type_check(i.types[0], imm) && operand_type_check(i.types[1], imm)))
		swap_operands();
	...
	if (sse_check != check_none && !i.tm.opcode_modifier.noavx && !i.tm.cpu_flags.bitfield.cpuavx && !i.tm.cpu_flags.bitfield.cpuavx512f && (i.tm.cpu_flags.bitfield.cpusse || i.tm.cpu_flags.bitfield.cpusse2 || i.tm.cpu_flags.bitfield.cpusse3 || i.tm.cpu_flags.bitfield.cpussse3 || i.tm.cpu_flags.bitfield.cpusse4_1 || i.tm.cpu_flags.bitfield.cpusse4_2 || i.tm.cpu_flags.bitfield.cpupclmul || i.tm.cpu_flags.bitfield.cpuaes || i.tm.cpu_flags.bitfield.cpusha || i.tm.cpu_flags.bitfield.cpugfni))
	{
	...
	if (i.prefix[LOCK_PREFIX] && (!i.tm.opcode_modifier.islockable || i.mem_operands == 0 || (i.tm.base_opcode != 0x86 && !(i.flags[i.operands - 1] & Operand_Mem))))
	{
	...
	if (i.prefix[DATA_PREFIX] && (is_any_vex_encoding(&i.tm) || i.tm.operand_types[i.imm_operands].bitfield.class_width >= RegMMX || i.tm.operand_types[i.imm_operands + 1].bitfield.class_width >= RegMMX))
	{
	...
	if (i.operands <= 3)
		for (j = 0; j < i.operands; j++)
			if (i.types[j].bitfield.instance != InstanceNone && !i.types[j].bitfield.xmmword)
				i.reg_operands--;
	...
	if ((i.tm.opcode_modifier.jump == JUMP || i.tm.opcode_modifier.jump == JUMP_BYTE || i.tm.opcode_modifier.jump == JUMP_DWORD) && i.op[0].disps->X_op == O_constant)
	{
	...
	if ((i.types[0].bitfield.class_width == Reg && i.types[0].bitfield.byte && (i.op[0].regs->reg_flags & RegRex64) != 0) || (i.types[1].bitfield.class_width == Reg && i.types[1].bitfield.byte && (i.op[1].regs->reg_flags & RegRex64) != 0) || (((i.types[0].bitfield.class_width == Reg && i.types[0].bitfield.byte) || (i.types[1].bitfield.class_width == Reg && i.types[1].bitfield.byte)) && i.rex != 0))
	{
		int x;
		...
		for (x = 0; x < 2; x++)
		{
			...
			if (i.types[x].bitfield.class_width == Reg && i.types[x].bitfield.byte && (i.op[x].regs->reg_flags & RegRex64) == 0)
			{
	...
	if (i.rex == 0 && i.rex_encoding)
	{
	 ...
	 the REX_OPCODE byte.  */
		int x;
		for (x = 0; x < 2; x++)
			if (i.types[x].bitfield.class_width == Reg && i.types[x].bitfield.byte && (i.op[x].regs->reg_flags & RegRex64) == 0 && i.op[x].regs->reg_num > 3)
			{
				gas_assert(!(i.op[x].regs->reg_flags & RegRex));
...
// 在op_hash查询这个助记符，返回对应的templates
		/* Look up instruction (or prefix) via hash table.  */
		current_templates = (const templates *)hash_find(op_hash, mnemonic);

		if (*l != END_OF_INSN && (!is_space_char(*l) || l[1] != END_OF_INSN) && current_templates && current_templates->start->opcode_modifier.isprefix)
		{
	     ...
	     Similarly, in 32-bit mode, do not allow addr32 or data32.  */
			if ((current_templates->start->opcode_modifier.size == SIZE16 || current_templates->start->opcode_modifier.size == SIZE32) && flag_code != CODE_64BIT && ((current_templates->start->opcode_modifier.size == SIZE32) ^ (flag_code == CODE_16BIT)))
			{
	...
	if (current_templates->start->opcode_modifier.jump == JUMP || current_templates->start->opcode_modifier.jump == JUMP_BYTE)
	{
	...
	else if (i.reg_operands)
	{
	 ...
	 immediates.  In any case, we can't set i.suffix yet.  */
		for (op = i.operands; --op >= 0;)
			if (i.types[op].bitfield.class_width != Reg)
				continue;
	...
	for (op = i.operands; --op >= 0;)
		if (operand_type_check(i.types[op], imm))
		{
	       ...
	       be recognised as within Imm8S range.  */
				if ((i.types[op].bitfield.imm16) && (i.op[op].imms->X_add_number & ~(offsetT)0xffff) == 0)
				{
				...
				if ((i.types[op].bitfield.imm32) && ((i.op[op].imms->X_add_number & ~(((offsetT)2 << 31) - 1)) == 0))
				{
					i.op[op].imms->X_add_number = ((i.op[op].imms->X_add_number ^ ((offsetT)1 << 31)) - ((offsetT)1 << 31));
				}
#endif
		...
		if (operand_type_check(i.types[op], disp))
		{
		   ...
		   displacement.  */
				...
				else if (flag_code == CODE_64BIT)
				{
					...
					if (i.prefix[ADDR_PREFIX] && fits_in_unsigned_long(op_disp))
						i.types[op].bitfield.disp32 = 1;
				}
				if ((i.types[op].bitfield.disp32 || i.types[op].bitfield.disp32s || i.types[op].bitfield.disp16) && fits_in_disp8(op_disp))
...
{
	return ((t->opcode_modifier.broadcast == BYTE_BROADCAST && i.types[given].bitfield.byte) || (t->opcode_modifier.broadcast == WORD_BROADCAST && i.types[given].bitfield.word) || (t->opcode_modifier.broadcast == DWORD_BROADCAST && i.types[given].bitfield.dword) || (t->opcode_modifier.broadcast == QWORD_BROADCAST && i.types[given].bitfield.qword));
}
...
check_VecOperands(const insn_template *t)
{
	...
	if (!cpu_flags_all_zero(&cpu) && !t->cpu_flags.bitfield.cpuavx512vl && !cpu_arch_flags.bitfield.cpuavx512vl)
	{
		for (op = 0; op < t->operands; ++op)
		{
			if (t->operand_types[op].bitfield.zmmword && (i.types[op].bitfield.ymmword || i.types[op].bitfield.xmmword))
			{
				i.error = unsupported;
				return 1;
			}
	...
	if (!t->opcode_modifier.sib && i.index_reg && (i.index_reg->reg_type.bitfield.xmmword || i.index_reg->reg_type.bitfield.ymmword || i.index_reg->reg_type.bitfield.zmmword))
	{
	...
	{
		if (!i.index_reg || !((t->opcode_modifier.sib == VECSIB128 && i.index_reg->reg_type.bitfield.xmmword) || (t->opcode_modifier.sib == VECSIB256 && i.index_reg->reg_type.bitfield.ymmword) || (t->opcode_modifier.sib == VECSIB512 && i.index_reg->reg_type.bitfield.zmmword)))
		{
		...
		{
			gas_assert(i.types[0].bitfield.class_width == RegSIMD);
			gas_assert(i.types[0].bitfield.xmmword || i.types[0].bitfield.ymmword);
			gas_assert(i.types[2].bitfield.class_width == RegSIMD);
			gas_assert(i.types[2].bitfield.xmmword || i.types[2].bitfield.ymmword);
			if (operand_check == check_none)
				return 0;
			if (register_number(i.op[0].regs) != register_number(i.index_reg) && register_number(i.op[2].regs) != register_number(i.index_reg) && register_number(i.op[0].regs) != register_number(i.op[2].regs))
				return 0;
		...
		{
			if (i.types[1].bitfield.class_width == RegSIMD && (i.types[1].bitfield.xmmword || i.types[1].bitfield.ymmword || i.types[1].bitfield.zmmword) && (register_number(i.op[1].regs) == register_number(i.index_reg)))
			{
				if (operand_check == check_error)
				{
					i.error = invalid_vector_register_set;
	...
	if (i.broadcast)
	{
	 ...
	 and its broadcast bytes match the memory operand.  */
		...
		if (!t->opcode_modifier.broadcast || !(i.flags[op] & Operand_Mem) || (!i.types[op].bitfield.unspecified && !match_broadcast_size(t, op)))
		{
		bad_broadcast:
		...
		i.broadcast->bytes = ((1 << (t->opcode_modifier.broadcast - 1)) * i.broadcast->type);
		...
		if (t->operand_types[op].bitfield.class_width == RegSIMD && t->operand_types[op].bitfield.byte + t->operand_types[op].bitfield.word + t->operand_types[op].bitfield.dword + t->operand_types[op].bitfield.qword > 1)
		{
		...
		if (t->opcode_modifier.checkregsize)
		{
			...
			for (j = 0; j < i.operands; ++j)
			{
				if (j != op && !operand_type_register_match(i.types[j],
															t->operand_types[j],
															type,
															t->operand_types[op]))
					goto bad_broadcast;
	...
	if (i.rounding)
	{
		if (!t->opcode_modifier.sae || (i.rounding->type != saeonly && !t->opcode_modifier.staticrounding))
		{
	...
	if (t->opcode_modifier.disp8memshift && i.disp_encoding != disp_encoding_32bit)
	{
		if (i.broadcast)
		...
		else
		{
			...
			for (op = 0; op < i.operands; op++)
				if (i.flags[op] & Operand_Mem)
				{
					...
					else if (t->operand_types[op].bitfield.xmmword + t->operand_types[op].bitfield.ymmword + t->operand_types[op].bitfield.zmmword <= 1)
						type = &t->operand_types[op];
					else if (!i.types[op].bitfield.unspecified)
						type = &i.types[op];
				}
				else if (i.types[op].bitfield.class_width == RegSIMD && t->opcode_modifier.evex != EVEXLIG)
				{
					if (i.types[op].bitfield.zmmword)
						i.memshift = 6;
		...
		for (op = 0; op < i.operands; op++)
			if (operand_type_check(i.types[op], disp) && i.op[op].disps->X_op == O_constant)
			{
...
#error "MAX_OPERANDS must be 5."
	...
	for (t = current_templates->start; t < current_templates->end; t++)
	{
		...
		if ((intel_syntax && t->opcode_modifier.attsyntax) || (!intel_syntax && t->opcode_modifier.intelsyntax))
			continue;
		...
		if ((t->opcode_modifier.no_bsuf && suffix_check.no_bsuf) || (t->opcode_modifier.no_wsuf && suffix_check.no_wsuf) || (t->opcode_modifier.no_lsuf && suffix_check.no_lsuf) || (t->opcode_modifier.no_ssuf && suffix_check.no_ssuf) || (t->opcode_modifier.no_qsuf && suffix_check.no_qsuf) || (t->opcode_modifier.no_ldsuf && suffix_check.no_ldsuf))
			continue;
	 ...
	 - 32-bit operands on pre-386.  */
		...
		if (((i.suffix == QWORD_MNEM_SUFFIX && flag_code != CODE_64BIT && (t->base_opcode != 0x0fc7 || t->extension_opcode != 1 /* cmpxchg8b */)) || (i.suffix == LONG_MNEM_SUFFIX && !cpu_arch_flags.bitfield.cpui386)) && (intel_syntax ? (t->opcode_modifier.mnemonicsize != IGNORESIZE && !intel_float_operand(t->name)) : intel_float_operand(t->name) != 2) && (t->operands == i.imm_operands || (operand_types[i.imm_operands].bitfield.class_width != RegMMX && operand_types[i.imm_operands].bitfield.class_width != RegSIMD && operand_types[i.imm_operands].bitfield.class_width != RegMask) || (operand_types[j].bitfield.class_width != RegMMX && operand_types[j].bitfield.class_width != RegSIMD && operand_types[j].bitfield.class_width != RegMask)) && !t->opcode_modifier.sib)
			continue;
		...
		if (!t->opcode_modifier.jump || t->opcode_modifier.jump == JUMP_ABSOLUTE)
		{
		 ...
		 operand into Disp32/Disp32/Disp16/Disp32 operand.  */
				...
				case CODE_16BIT:
					...
					/* Fall through.  */
				case CODE_32BIT:
					if (operand_types[j].bitfield.disp32 && operand_types[j].bitfield.disp16)
					{
				...
				case CODE_64BIT:
					if (operand_types[j].bitfield.disp32s || operand_types[j].bitfield.disp64)
					{
		...
		case 2:
	     ...
	     zero-extend %eax to %rax.  */
			if (flag_code == CODE_64BIT && t->base_opcode == 0x90 && i.types[0].bitfield.instance == Accum && i.types[0].bitfield.dword && i.types[1].bitfield.instance == Accum && i.types[1].bitfield.dword)
				continue;
			/* xrelease mov %eax, <disp> is another special case. It must not
	     match the accumulator-only encoding of mov.  */
			if (flag_code != CODE_64BIT && i.hle_prefix && t->base_opcode == 0xa0 && i.types[0].bitfield.instance == Accum && (i.flags[1] & Operand_Mem))
				continue;
			/* Fall through.  */
		...
		case 3:
	     ...
	     - the non-default (swapped) form is requested.  */
			...
			if (t->opcode_modifier.d && i.reg_operands == i.operands && !operand_type_all_zero(&overlap1))
				switch (i.dir_encoding)
				{
				case dir_encoding_load:
					if (operand_type_check(operand_types[i.operands - 1], anymem) || t->opcode_modifier.regmem)
						goto check_reverse;
					break;
			...
			if ((i.dir_encoding == dir_encoding_store || i.dir_encoding == dir_encoding_swap) && i.mem_operands == 0 && t->opcode_modifier.load)
				continue;
			/* Fall through.  */
		case 4:
		case 5:
			overlap1 = operand_type_and(i.types[1], operand_types[1]);
			if (!operand_type_match(overlap0, i.types[0]) || !operand_type_match(overlap1, i.types[1]) || ((check_register & 3) == 3 && !operand_type_register_match(i.types[0], operand_types[0], i.types[1], operand_types[1])))
			{
			...
			check_reverse:
				...
				if (!operand_type_match(overlap0, i.types[0]) || !operand_type_match(overlap1, i.types[i.operands - 1]) || (check_register && !operand_type_register_match(i.types[0], operand_types[i.operands - 1], i.types[i.operands - 1], operand_types[0])))
				{
		 ...
		 we've found.  */
				...
				else if (operand_types[0].bitfield.xmmword || operand_types[i.operands - 1].bitfield.xmmword || operand_types[0].bitfield.class_width == RegMMX || operand_types[i.operands - 1].bitfield.class_width == RegMMX || is_any_vex_encoding(t))
			...
			else
			{
				...
				{
				case 5:
					if (!operand_type_match(overlap4, i.types[4]) || !operand_type_register_match(i.types[3],
																								  operand_types[3],
																								  i.types[4],
																								  operand_types[4]))
						continue;
					/* Fall through.  */
				case 4:
					if (!operand_type_match(overlap3, i.types[3]) || ((check_register & 0xa) == 0xa && !operand_type_register_match(i.types[1], operand_types[1], i.types[3], operand_types[3])) || ((check_register & 0xc) == 0xc && !operand_type_register_match(i.types[2], operand_types[2], i.types[3], operand_types[3])))
						continue;
					/* Fall through.  */
				case 3:
					/* Here we make use of the fact that there are no
		     reverse match 3 operand instructions.  */
					if (!operand_type_match(overlap2, i.types[2]) || ((check_register & 5) == 5 && !operand_type_register_match(i.types[0], operand_types[0], i.types[2], operand_types[2])) || ((check_register & 6) == 6 && !operand_type_register_match(i.types[1], operand_types[1], i.types[2], operand_types[2])))
						continue;
					break;
				}
			}
	...
	if (!quiet_warnings)
	{
		if (!intel_syntax && (i.jumpabsolute != (t->opcode_modifier.jump == JUMP_ABSOLUTE)))
			as_warn(_("indirect %s without `*'"), t->name);

		if (t->opcode_modifier.isprefix && t->opcode_modifier.mnemonicsize == IGNORESIZE)
		{
	...
	if (addr_prefix_disp != -1)
		i.tm.operand_types[addr_prefix_disp] = operand_types[addr_prefix_disp];
	...
	if (found_reverse_match)
	{
	 ...
	 We need to avoid setting the bit though on insns like KMOVW.  */
		i.tm.opcode_modifier.regmem = i.tm.opcode_modifier.modrm && i.tm.opcode_modifier.d && i.tm.operands > 2U - i.tm.opcode_modifier.sse2avx && !i.tm.opcode_modifier.regmem;
	}
...
process_suffix(void)
{
	...
	else if (i.reg_operands && (i.operands > 1 || i.types[0].bitfield.class_width == Reg) && !i.tm.opcode_modifier.addrprefixopreg)
	{
	 ...
	 to represent the destination (register).  */
		if (((i.tm.base_opcode | 8) == 0xfbe && i.tm.opcode_modifier.w) || (i.tm.base_opcode == 0x63 && i.tm.cpu_flags.bitfield.cpu64))
			--i.operands;

		/* crc32 needs REX.W set regardless of suffix / source operand size.  */
		if (i.tm.base_opcode == 0xf20f38f0 && i.tm.operand_types[1].bitfield.qword)
			i.rex |= REX_W;
	 ...
	 based on GPR operands.  */
		if (!i.suffix)
		{
	     ...
	     type. */
			...
			while (op--)
				if (i.tm.operand_types[op].bitfield.instance == InstanceNone || i.tm.operand_types[op].bitfield.instance == Accum)
				{
					if (i.types[op].bitfield.class_width != Reg)
						continue;
	     ...
	     in AT&T mode.  */
			if ((i.tm.base_opcode | 8) == 0xfbe && i.tm.opcode_modifier.w && !i.suffix && !intel_syntax)
				i.suffix = BYTE_MNEM_SUFFIX;
		}
		else if (i.suffix == BYTE_MNEM_SUFFIX)
		{
			if (intel_syntax && i.tm.opcode_modifier.mnemonicsize == IGNORESIZE && i.tm.opcode_modifier.no_bsuf)
				i.suffix = 0;
		...
		{
			if (intel_syntax && i.tm.opcode_modifier.mnemonicsize == IGNORESIZE && i.tm.opcode_modifier.no_lsuf && !i.tm.opcode_modifier.todword && !i.tm.opcode_modifier.toqword)
				i.suffix = 0;
		...
		{
			if (intel_syntax && i.tm.opcode_modifier.mnemonicsize == IGNORESIZE && i.tm.opcode_modifier.no_qsuf && !i.tm.opcode_modifier.todword && !i.tm.opcode_modifier.toqword)
				i.suffix = 0;
		...
		{
			if (intel_syntax && i.tm.opcode_modifier.mnemonicsize == IGNORESIZE && i.tm.opcode_modifier.no_wsuf)
				i.suffix = 0;
			else if (!check_word_reg())
				return 0;
		}
		else if (intel_syntax && i.tm.opcode_modifier.mnemonicsize == IGNORESIZE)
	...
	else if (i.tm.opcode_modifier.mnemonicsize == DEFAULTSIZE && !i.suffix)
	{
	...
	else if (!i.suffix && (i.tm.opcode_modifier.jump == JUMP_ABSOLUTE || i.tm.opcode_modifier.jump == JUMP_BYTE || i.tm.opcode_modifier.jump == JUMP_INTERSEGMENT || (i.tm.base_opcode == 0x0f01 /* [ls][gi]dt */
																																									  && i.tm.extension_opcode <= 3)))
	{
		...
		case CODE_64BIT:
			...
			{
				if (i.tm.opcode_modifier.jump == JUMP_BYTE || i.tm.opcode_modifier.no_lsuf)
					i.suffix = QWORD_MNEM_SUFFIX;
				break;
			}
			/* Fall through.  */
	...
	if (!i.suffix && (i.tm.opcode_modifier.mnemonicsize != DEFAULTSIZE
					  /* Also cover lret/retf/iret in 64-bit mode.  */
					  || (flag_code == CODE_64BIT && !i.tm.opcode_modifier.no_lsuf && !i.tm.opcode_modifier.no_qsuf)) &&
		...
		&& (i.tm.opcode_modifier.no_ssuf || i.tm.opcode_modifier.floatmf))
	{
	 ...
	 restricted to just Intel syntax.  */
		if (intel_syntax && is_any_vex_encoding(&i.tm) && !i.broadcast)
		{
			...
			{
				if (is_evex_encoding(&i.tm) && !cpu_arch_flags.bitfield.cpuavx512vl)
				{
				...
				if (i.tm.operand_types[op].bitfield.xmmword + i.tm.operand_types[op].bitfield.ymmword + i.tm.operand_types[op].bitfield.zmmword < 2)
					continue;

				/* Any properly sized operand disambiguates the insn.  */
				if (i.types[op].bitfield.xmmword || i.types[op].bitfield.ymmword || i.types[op].bitfield.zmmword)
				{
					suffixes &= ~(7 << 6);
					evex = 0;
					break;
				...
				if ((i.flags[op] & Operand_Mem) && i.tm.operand_types[op].bitfield.unspecified)
				{
		...
		{
			if (intel_syntax && (i.tm.opcode_modifier.mnemonicsize != DEFAULTSIZE || operand_check == check_error))
			{
			...
			if (operand_check == check_error)
			{
				as_bad(_("no instruction mnemonic suffix given and "
						 "no register operands; can't size `%s'"),
					   i.tm.name);
				return 0;
			}
			...
			if (i.tm.opcode_modifier.floatmf)
				i.suffix = SHORT_MNEM_SUFFIX;
			else if ((i.tm.base_opcode | 8) == 0xfbe || (i.tm.base_opcode == 0x63 && i.tm.cpu_flags.bitfield.cpu64))
				/* handled below */;
			else if (evex)
	...
	if ((i.tm.base_opcode | 8) == 0xfbe || (i.tm.base_opcode == 0x63 && i.tm.cpu_flags.bitfield.cpu64))
	{
	...
	if (!i.tm.opcode_modifier.modrm && i.reg_operands && i.tm.operands < 3)
		i.short_form = (i.tm.operand_types[0].bitfield.class_width == Reg) != (i.tm.operand_types[1].bitfield.class_width == Reg);
	...
	case SHORT_MNEM_SUFFIX:
	 ...
	 prefix anyway.  */
		if (i.suffix != QWORD_MNEM_SUFFIX && i.tm.opcode_modifier.mnemonicsize != IGNORESIZE && !i.tm.opcode_modifier.floatmf && !is_any_vex_encoding(&i.tm) && ((i.suffix == LONG_MNEM_SUFFIX) == (flag_code == CODE_16BIT) || (flag_code == CODE_64BIT && i.tm.opcode_modifier.jump == JUMP_BYTE)))
		{
		...
		if (i.suffix == QWORD_MNEM_SUFFIX && flag_code == CODE_64BIT && !i.tm.opcode_modifier.norex64 && !i.tm.opcode_modifier.vexw
			/* Special case for xchg %rax,%rax.  It is NOP and doesn't
	     need rex64. */
			&& !(i.operands == 2 && i.tm.base_opcode == 0x90 && i.tm.extension_opcode == None && i.types[0].bitfield.instance == Accum && i.types[0].bitfield.qword && i.types[1].bitfield.instance == Accum && i.types[1].bitfield.qword))
			i.rex |= REX_W;
	...
	case 0:
	 ...
	 when there are no suitable register operands.  */
		if (i.tm.opcode_modifier.w && (i.prefix[DATA_PREFIX] || (i.prefix[REX_PREFIX] & REX_W)) && (!i.reg_operands || (i.reg_operands == 1
																														/* ShiftCount */
																														&& (i.tm.operand_types[0].bitfield.instance == RegC
																															/* InOutPortReg */
																															|| i.tm.operand_types[0].bitfield.instance == RegD || i.tm.operand_types[1].bitfield.instance == RegD
																															/* CRC32 */
	...
	if (i.tm.opcode_modifier.addrprefixopreg)
	{
		...
		if (i.tm.operand_types[0].bitfield.instance == Accum || i.operands == 1)
		{
			/* The address size override prefix changes the size of the
	     first operand.  */
			if (flag_code == CODE_64BIT && i.op[0].regs->reg_type.bitfield.word)
			{
...
check_byte_reg(void)
{
	...
	for (op = i.operands; --op >= 0;)
	{
	 ...
	 low portion, and that's OK too.  */
		...
		if (i.tm.operand_types[op].bitfield.instance == RegD && i.tm.operand_types[op].bitfield.word)
			continue;
...
check_long_reg(void)
{
	...
	for (op = i.operands; --op >= 0;)
       ...
       them. (eg. movzb)  */
		else if (i.types[op].bitfield.byte && (i.tm.operand_types[op].bitfield.class_width == Reg || i.tm.operand_types[op].bitfield.instance == Accum) && (i.tm.operand_types[op].bitfield.word || i.tm.operand_types[op].bitfield.dword))
		{
			as_bad(_("`%s%s' not allowed with `%s%c'"),
				   register_prefix,
		...
		else if (i.types[op].bitfield.word && (i.tm.operand_types[op].bitfield.class_width == Reg || i.tm.operand_types[op].bitfield.instance == Accum) && i.tm.operand_types[op].bitfield.dword)
		{
		...
		else if (i.types[op].bitfield.qword && (i.tm.operand_types[op].bitfield.class_width == Reg || i.tm.operand_types[op].bitfield.instance == Accum) && i.tm.operand_types[op].bitfield.dword)
		{
			if (intel_syntax && i.tm.opcode_modifier.toqword && i.types[0].bitfield.class_width != RegSIMD)
			{
...
check_qword_reg(void)
{
	...
	for (op = i.operands; --op >= 0;)
       ...
       them. (eg. movzb)  */
		...
		else if ((i.types[op].bitfield.word || i.types[op].bitfield.dword) && (i.tm.operand_types[op].bitfield.class_width == Reg || i.tm.operand_types[op].bitfield.instance == Accum) && i.tm.operand_types[op].bitfield.qword)
		{
			/* Prohibit these changes in the 64bit mode, since the
	   lowering is more complicated.  */
			if (intel_syntax && i.tm.opcode_modifier.todword && i.types[0].bitfield.class_width != RegSIMD)
			{
...
check_word_reg(void)
{
	...
	for (op = i.operands; --op >= 0;)
       ...
       them. (eg. movzb)  */
		...
		else if ((i.types[op].bitfield.dword || i.types[op].bitfield.qword) && (i.tm.operand_types[op].bitfield.class_width == Reg || i.tm.operand_types[op].bitfield.instance == Accum) && i.tm.operand_types[op].bitfield.word)
		{
			as_bad(_("incorrect register `%s%s' used with `%c' suffix"),
				   register_prefix, i.op[op].regs->reg_name,
				   i.suffix);
...
update_imm(unsigned int j)
{
	...
	if ((overlap.bitfield.imm8 || overlap.bitfield.imm8s || overlap.bitfield.imm16 || overlap.bitfield.imm32 || overlap.bitfield.imm32s || overlap.bitfield.imm64) && !operand_type_equal(&overlap, &imm8) && !operand_type_equal(&overlap, &imm8s) && !operand_type_equal(&overlap, &imm16) && !operand_type_equal(&overlap, &imm32) && !operand_type_equal(&overlap, &imm32s) && !operand_type_equal(&overlap, &imm64))
	{
		...
		else if (operand_type_equal(&overlap, &imm16_32_32s) || operand_type_equal(&overlap, &imm16_32) || operand_type_equal(&overlap, &imm16_32s))
		{
...
process_operands(void)
{
	...
	if (i.tm.opcode_modifier.sse2avx && i.tm.opcode_modifier.vexvvvv)
	{
		...
		if (i.tm.operand_types[0].bitfield.instance == Accum && i.tm.operand_types[0].bitfield.xmmword)
		{
		 ...
		 sources.  */
				...
				i.tm.operand_types[0].bitfield.class_width = RegSIMD;
				goto duplicate;
			}
			else
			{
	...
	else if (i.tm.operand_types[0].bitfield.instance == Accum && i.tm.operand_types[0].bitfield.xmmword)
	{
		unsigned int j;
	...
	else if (i.tm.opcode_modifier.implicitquadgroup)
	{
		...
		gas_assert(i.operands >= 2 && i.types[1].bitfield.class_width == RegSIMD);
	...
	else if (i.types[0].bitfield.class_width == SReg)
	{
		if (flag_code != CODE_64BIT
				? i.tm.base_opcode == POP_SEG_SHORT && i.op[0].regs->reg_num == 1
				: (i.tm.base_opcode | 1) == POP_SEG386_SHORT && i.op[0].regs->reg_num < 4)
		{
	...
	else if (i.short_form)
	{
	 ...
	 0 or 1.  */
		unsigned int op = i.tm.operand_types[0].bitfield.class_width != Reg;
	...
	if ((i.seg[0] || i.prefix[SEG_PREFIX]) && i.tm.base_opcode == 0x8d /* lea */
	...
	if (i.seg[0] && i.seg[0] != default_seg && i.seg[0]->seg_prefix != i.prefix[SEG_PREFIX])
	{
...
build_modrm_byte(void)
{
	...
	if (vex_3_sources)
	{
	 ...
	 plus 1 memory operand, with VexXDS.  */
		gas_assert((i.reg_operands == 4 || (i.reg_operands == 3 && i.mem_operands == 1)) && i.tm.opcode_modifier.vexvvvv == VEXXDS && i.tm.opcode_modifier.vexw && i.tm.operand_types[dest].bitfield.class_width == RegSIMD);
	 ...
	 the second non-immediate one is encoded in the immediate operand.  */
		...
		if (i.imm_operands == 0)
		{
	     ...
	     immediate operand to encode the first operand.  */
			...
			gas_assert(i.tm.operand_types[reg_slot].bitfield.class_width == RegSIMD);
			exp->X_op = O_constant;
		...
		else
		{
			...
			gas_assert(i.tm.operand_types[reg_slot].bitfield.class_width == RegSIMD);
			i.op[0].imms->X_add_number |= register_number(i.op[reg_slot].regs) << 4;
			gas_assert((i.op[reg_slot].regs->reg_flags & RegVRex) == 0);
		}

		gas_assert(i.tm.operand_types[nds].bitfield.class_width == RegSIMD);
	...
	if (i.mem_operands == 0 && ((i.reg_operands == 2 && i.tm.opcode_modifier.vexvvvv <= VEXXDS) || (i.reg_operands == 3 && i.tm.opcode_modifier.vexvvvv == VEXXDS) || (i.reg_operands == 4 && vex_3_sources)))
	{
		switch (i.operands)
		{
		case 2:
		...
		case 3:
	     ...
	     is an instruction with VexNDS. */
			gas_assert(i.imm_operands == 1 || (i.imm_operands == 0 && (i.tm.opcode_modifier.vexvvvv == VEXXDS || (i.types[0].bitfield.instance == RegC && i.types[0].bitfield.byte))));
			if (operand_type_check(i.types[0], imm) || (i.types[0].bitfield.instance == RegC && i.types[0].bitfield.byte))
				source = 1;
			else
				source = 0;
			break;
		case 4:
	     ...
	     operand is imm8, the source operand is the first one.  */
			gas_assert((i.imm_operands == 2 && i.types[0].bitfield.imm8 && i.types[1].bitfield.imm8) || (i.tm.opcode_modifier.vexvvvv == VEXXDS && i.imm_operands == 1 && (i.types[0].bitfield.imm8 || i.types[i.operands - 1].bitfield.imm8 || i.rounding)));
			if (i.imm_operands == 2)
				source = 2;
			else
			{
		...
		if (!vex_3_sources)
		{
		 ...
		 register.  It is encoded in VEX prefix.  */
				...
				if ((dest + 1) >= i.operands || ((op.bitfield.class_width != Reg || (!op.bitfield.dword && !op.bitfield.qword)) && op.bitfield.class_width != RegSIMD && !operand_type_equal(&op, &regmask)))
					abort();
	 ...
	 destination in the i.rm.reg field.  */
		if (!i.tm.opcode_modifier.regmem && operand_type_check(i.tm.operand_types[dest], anymem) == 0)
		{
			i.rm.reg = i.op[dest].regs->reg_num;
			i.rm.regmem = i.op[source].regs->reg_num;
			if (i.op[dest].regs->reg_type.bitfield.class_width == RegMMX || i.op[source].regs->reg_type.bitfield.class_width == RegMMX)
				i.has_regmmx = TRUE;
			else if (i.op[dest].regs->reg_type.bitfield.class_width == RegSIMD || i.op[source].regs->reg_type.bitfield.class_width == RegSIMD)
			{
				if (i.types[dest].bitfield.zmmword || i.types[source].bitfield.zmmword)
					i.has_regzmm = TRUE;
				else if (i.types[dest].bitfield.ymmword || i.types[source].bitfield.ymmword)
					i.has_regymm = TRUE;
				else
					i.has_regxmm = TRUE;
			}
		...
		{
			if (i.types[!i.tm.opcode_modifier.regmem].bitfield.class_width != RegCR)
				abort();
			i.rex &= ~REX_R;
	...
	{ /* If it's not 2 reg operands...  */
		...
		if (i.mem_operands)
		{
		 ...
		 complications.  */
				if (!(i.base_reg->reg_flags & RegRex) && (i.base_reg->reg_num == EBP_REG_NUM || i.base_reg->reg_num == ESP_REG_NUM))
					default_seg = &ss;
	 ...
	 registers are coded into the i.rm.reg field.  */
		else if (i.reg_operands)
		{
			...
			for (op = 0; op < i.operands; op++)
			{
				if (i.types[op].bitfield.class_width == Reg || i.types[op].bitfield.class_width == RegBND || i.types[op].bitfield.class_width == RegMask || i.types[op].bitfield.class_width == SReg || i.types[op].bitfield.class_width == RegCR || i.types[op].bitfield.class_width == RegDR || i.types[op].bitfield.class_width == RegTR)
					break;
				if (i.types[op].bitfield.class_width == RegSIMD)
				{
					if (i.types[op].bitfield.zmmword)
						i.has_regzmm = TRUE;
					else if (i.types[op].bitfield.ymmword)
				...
				if (i.types[op].bitfield.class_width == RegMMX)
				{
					i.has_regmmx = TRUE;
					break;
				}
		 ...
		 source operand is encoded in VEX prefix. */
		     ...
		     operands are swapped.  */
					if (!i.tm.operand_types[op].bitfield.baseindex && i.tm.operand_types[op + 1].bitfield.baseindex)
					{
						vex_reg = op;
						op += 2;
						gas_assert(mem == (vex_reg + 1) && op < i.operands);
					}
					else
					{
						vex_reg = op + 1;
		 ...
		 is encoded in VEX prefix.  */
				...
				else
				{
					...
					gas_assert(op < i.imm_operands + 2 && i.operands == i.imm_operands + 2);
			...
			if (vex_reg != (unsigned int)~0)
			{
				...
				if ((type->bitfield.class_width != Reg || (!type->bitfield.dword && !type->bitfield.qword)) && type->bitfield.class_width != RegSIMD && !operand_type_equal(type, &regmask))
					abort();
...
void x86_cleanup(void)
{
   ...
   */
	...
	bfd_set_section_flags(sec,
						  (SEC_ALLOC | SEC_LOAD | SEC_DATA | SEC_HAS_CONTENTS | SEC_READONLY));
	...
	feature_2_descsz = ((feature_2_descsz_raw + align_size_1) & ~align_size_1);
...
maybe_fused_with_jcc_p(enum mf_cmp_kind *mf_cmp_p)
{
	...
	if (i.tm.base_opcode <= 5 || (i.tm.base_opcode >= 0x28 && i.tm.base_opcode <= 0x2d) || ((i.tm.base_opcode | 3) == 0x83 && (i.tm.extension_opcode == 0x5 || i.tm.extension_opcode == 0x0)))
	{
	...
	if ((i.tm.base_opcode >= 0x20 && i.tm.base_opcode <= 0x25) || ((i.tm.base_opcode | 3) == 0x83 && i.tm.extension_opcode == 0x4))
	{
	...
	if ((i.tm.base_opcode | 1) == 0x85 || (i.tm.base_opcode | 1) == 0xa9 || ((i.tm.base_opcode | 1) == 0xf7 && i.tm.extension_opcode == 0))
	{
		*mf_cmp_p = mf_cmp_test_and;
		return !(i.mem_operands && i.imm_operands);
	}
	...
	if ((i.tm.base_opcode >= 0x38 && i.tm.base_opcode <= 0x3d) || ((i.tm.base_opcode | 3) == 0x83 && (i.tm.extension_opcode == 0x7)))
	{
		*mf_cmp_p = mf_cmp_alu_cmp;
		return !(i.mem_operands && i.imm_operands);
	}
	...
	if ((i.tm.cpu_flags.bitfield.cpuno64 && (i.tm.base_opcode | 0xf) == 0x4f) || ((i.tm.base_opcode | 1) == 0xff && i.tm.extension_opcode <= 0x1))
	{
...
void x86_cons_fix_new(fragS *frag, unsigned int off, unsigned int len,
...
#if !(defined(OBJ_ELF) || defined(OBJ_MAYBE_ELF) || defined(OBJ_MACH_O)) || defined(LEX_AT)
...
#if defined(OBJ_ELF) || defined(OBJ_MAYBE_ELF)
		{STRING_COMMA_LEN("SIZE"), {BFD_RELOC_SIZE32, BFD_RELOC_SIZE32}, OPERAND_TYPE_IMM32_64},
#endif
		{STRING_COMMA_LEN("PLTOFF"), {_dummy_first_bfd_reloc_code_real, BFD_RELOC_X86_64_PLTOFF64}, OPERAND_TYPE_IMM64},
		{STRING_COMMA_LEN("PLT"), {BFD_RELOC_386_PLT32, BFD_RELOC_X86_64_PLT32}, OPERAND_TYPE_IMM32_32S_DISP32},
		{STRING_COMMA_LEN("GOTPLT"), {_dummy_first_bfd_reloc_code_real, BFD_RELOC_X86_64_GOTPLT64}, OPERAND_TYPE_IMM64_DISP64},
		{STRING_COMMA_LEN("GOTOFF"), {BFD_RELOC_386_GOTOFF, BFD_RELOC_X86_64_GOTOFF64}, OPERAND_TYPE_IMM64_DISP64},
		{STRING_COMMA_LEN("GOTPCREL"), {_dummy_first_bfd_reloc_code_real, BFD_RELOC_X86_64_GOTPCREL}, OPERAND_TYPE_IMM32_32S_DISP32},
		{STRING_COMMA_LEN("TLSGD"), {BFD_RELOC_386_TLS_GD, BFD_RELOC_X86_64_TLSGD}, OPERAND_TYPE_IMM32_32S_DISP32},
		{STRING_COMMA_LEN("TLSLDM"), {BFD_RELOC_386_TLS_LDM, _dummy_first_bfd_reloc_code_real}, OPERAND_TYPE_NONE},
		{STRING_COMMA_LEN("TLSLD"), {_dummy_first_bfd_reloc_code_real, BFD_RELOC_X86_64_TLSLD}, OPERAND_TYPE_IMM32_32S_DISP32},
		{STRING_COMMA_LEN("GOTTPOFF"), {BFD_RELOC_386_TLS_IE_32, BFD_RELOC_X86_64_GOTTPOFF}, OPERAND_TYPE_IMM32_32S_DISP32},
		{STRING_COMMA_LEN("TPOFF"), {BFD_RELOC_386_TLS_LE_32, BFD_RELOC_X86_64_TPOFF32}, OPERAND_TYPE_IMM32_32S_64_DISP32_64},
		{STRING_COMMA_LEN("NTPOFF"), {BFD_RELOC_386_TLS_LE, _dummy_first_bfd_reloc_code_real}, OPERAND_TYPE_NONE},
		{STRING_COMMA_LEN("DTPOFF"), {BFD_RELOC_386_TLS_LDO_32, BFD_RELOC_X86_64_DTPOFF32}, OPERAND_TYPE_IMM32_32S_64_DISP32_64},
		{STRING_COMMA_LEN("GOTNTPOFF"), {BFD_RELOC_386_TLS_GOTIE, _dummy_first_bfd_reloc_code_real}, OPERAND_TYPE_NONE},
		{STRING_COMMA_LEN("INDNTPOFF"), {BFD_RELOC_386_TLS_IE, _dummy_first_bfd_reloc_code_real}, OPERAND_TYPE_NONE},
		{STRING_COMMA_LEN("GOT"), {BFD_RELOC_386_GOT32, BFD_RELOC_X86_64_GOT32}, OPERAND_TYPE_IMM32_32S_64_DISP32},
		{STRING_COMMA_LEN("TLSDESC"), {BFD_RELOC_386_TLS_GOTDESC, BFD_RELOC_X86_64_GOTPC32_TLSDESC}, OPERAND_TYPE_IMM32_32S_DISP32},
		{STRING_COMMA_LEN("TLSCALL"), {BFD_RELOC_386_TLS_DESC_CALL, BFD_RELOC_X86_64_TLSDESC_CALL}, OPERAND_TYPE_IMM32_32S_DISP32},
	};
	char *cp;
	unsigned int j;
		...
		i386_operand_type *types)
{
	static const struct
	{
		...
		{
			{STRING_COMMA_LEN("SECREL32"), {BFD_RELOC_32_SECREL, BFD_RELOC_32_SECREL}, OPERAND_TYPE_IMM32_32S_64_DISP32_64},
		};
	...
	if (size == 4 || (object_64bit && size == 8))
	{
		...
		if (gotfree_input_line)
		{
	     ...
	     input_line_pointer has moved to the right buffer.  */
			input_line_pointer = (save + (input_line_pointer - gotfree_input_line) + adjust);
			...
			else if ((got_reloc == BFD_RELOC_386_PLT32 || got_reloc == BFD_RELOC_X86_64_PLT32) && exp->X_op != O_symbol)
			{
				char c = *input_line_pointer;
				*input_line_pointer = 0;
	...
	while (*op_string && (op_end == NULL || op_string < op_end))
	{
		...
		if (*op_string == '{')
		{
			...
			else if ((mask = parse_register(op_string, &end_op)) != NULL)
			{
				...
				if (mask->reg_type.bitfield.class_width != RegMask || !mask->reg_num)
				{
...
i386_displacement(char *disp_start, char *disp_end)
{
	...
	if (i.jumpabsolute || i.types[this_operand].bitfield.baseindex || (current_templates->start->opcode_modifier.jump != JUMP && current_templates->start->opcode_modifier.jump != JUMP_DWORD))
	{
	...
	else
	{
	 ...
	 branch template.  */
		...
		{
			if (t->opcode_modifier.jump != current_templates->start->opcode_modifier.jump)
				break;
			if ((t->opcode_modifier.isa64 >= INTEL64))
				has_intel64 = TRUE;
		}
		...
		if (flag_code == CODE_64BIT)
		{
			if ((override || i.suffix == WORD_MNEM_SUFFIX) && (!intel64 || !has_intel64))
				bigdisp.bitfield.disp16 = 1;
			else
				bigdisp.bitfield.disp32s = 1;
		}
						   ...
						   i386_operand_type types, const char *disp_start)
{
	...
	if (i.reloc[this_operand] == BFD_RELOC_386_GOTOFF || i.reloc[this_operand] == BFD_RELOC_X86_64_GOTPCREL || i.reloc[this_operand] == BFD_RELOC_X86_64_GOTOFF64)
	{
		...
		if (S_IS_LOCAL(exp->X_add_symbol) && S_GET_SEGMENT(exp->X_add_symbol) != undefined_section && S_GET_SEGMENT(exp->X_add_symbol) != expr_section)
	...
	else if (flag_code == CODE_64BIT && !i.prefix[ADDR_PREFIX] && exp->X_op == O_constant)
	{
...
i386_addressing_mode(void)
{
	...
	else if (flag_code == CODE_16BIT && current_templates->start->cpu_flags.bitfield.cpumpx
		...
		if (i.mem_operands == 0)
		{
			...
			if (addr_reg)
			{
				if (addr_reg->reg_type.bitfield.dword)
					addr_mode = CODE_32BIT;
				else if (flag_code != CODE_64BIT && addr_reg->reg_type.bitfield.word)
		     ...
		     up the code quite a lot.  */
					if (flag_code != CODE_64BIT && (i.types[this_operand].bitfield.disp16 || i.types[this_operand].bitfield.disp32))
						i.types[this_operand] = operand_type_xor(i.types[this_operand], disp16_32);
				}
			}
		}
#endif
...
i386_index_check(const char *operand_string)
{
	...
	if (current_templates->start->opcode_modifier.isstring && !current_templates->start->cpu_flags.bitfield.cpupadlock && (current_templates->end[-1].opcode_modifier.isstring || i.mem_operands))
	{
	 ...
	 a single register (rDI, rSI, or rBX) as their memory address.  */
		...
		{
			int es_op = current_templates->end[-1].opcode_modifier.isstring - IS_STRING_ES_OP0;
			int op = 0;

			if (!current_templates->end[-1].operand_types[0].bitfield.baseindex || ((!i.mem_operands != !intel_syntax) && current_templates->end[-1].operand_types[1].bitfield.baseindex))
				op = 1;
		...
		if (i.base_reg != expected_reg || i.index_reg || operand_type_check(i.types[this_operand], disp))
		{
	     ...
	     the first one.  */
			if (i.mem_operands && i.base_reg && !((addr_mode == CODE_64BIT && i.base_reg->reg_type.bitfield.qword) || (addr_mode == CODE_32BIT ? i.base_reg->reg_type.bitfield.dword : i.base_reg->reg_type.bitfield.word)))
				goto bad_address;
	...
	else
	{
		if (addr_mode != CODE_16BIT)
		{
			...
			if ((i.base_reg && ((addr_mode == CODE_64BIT
									 ? !i.base_reg->reg_type.bitfield.qword
									 : !i.base_reg->reg_type.bitfield.dword) ||
								(i.index_reg && i.base_reg->reg_num == RegIP) || i.base_reg->reg_num == RegIZ)) ||
				(i.index_reg && !i.index_reg->reg_type.bitfield.xmmword && !i.index_reg->reg_type.bitfield.ymmword && !i.index_reg->reg_type.bitfield.zmmword && ((addr_mode == CODE_64BIT ? !i.index_reg->reg_type.bitfield.qword : !i.index_reg->reg_type.bitfield.dword) || !i.index_reg->reg_type.bitfield.baseindex)))
				goto bad_address;

			/* bndmk, bndldx, and bndstx have special restrictions. */
			if (current_templates->start->base_opcode == 0xf30f1b || (current_templates->start->base_opcode & ~1) == 0x0f1a)
			{
				...
				if (current_templates->start->base_opcode != 0xf30f1b && i.log2_scale_factor)
		...
		{
			/* 16-bit checks.  */
			if ((i.base_reg && (!i.base_reg->reg_type.bitfield.word || !i.base_reg->reg_type.bitfield.baseindex)) || (i.index_reg && (!i.index_reg->reg_type.bitfield.word || !i.index_reg->reg_type.bitfield.baseindex || !(i.base_reg && i.base_reg->reg_num < 6 && i.index_reg->reg_num >= 6 && i.log2_scale_factor == 0))))
				goto bad_address;
		}
	}
	return 1;
...
maybe_adjust_templates(void)
{
	...
	if (t < current_templates->end)
	{
		...
		recheck = (aux_templates.start != current_templates->start || t != current_templates->end);
		...
		{
			i.mem_operands = 0;
			if (i.memop1_string != NULL && i386_index_check(i.memop1_string) == 0)
				return 0;
			i.mem_operands = 1;
		}
	}
...
i386_att_operand(char *operand_string)
{
	...
	if ((r = parse_register(op_string, &end_op)) != NULL)
	{
	 ...
	 segment register.  */
		...
		if (is_space_char(*op_string))
			++op_string;
		if (*op_string == ':' && r->reg_type.bitfield.class_width == SReg)
		{
			switch (r->reg_num)
			{
			case 0:
			...
			if (!is_digit_char(*op_string) && !is_identifier_char(*op_string) && *op_string != '(' && *op_string != ABSOLUTE_PREFIX)
			{
	...
	else if (is_digit_char(*op_string) || is_identifier_char(*op_string) || *op_string == '"' || *op_string == '(')
	{
	...
	do_memory_reference:
		if (i.mem_operands == 1 && !maybe_adjust_templates())
			return 0;
		if ((i.mem_operands == 1 && !current_templates->start->opcode_modifier.isstring) || i.mem_operands == 2)
		{
			as_bad(_("too many memory references for `%s'"),
				   current_templates->start->name);
			return 0;
	 ...
	 after the '('.  */
		...
		if (*base_string == ')')
		{
	     ...
	     equal, so this loop will not be infinite.  */
			...
			if (*base_string == ',' || ((i.base_reg = parse_register(base_string, &end_op)) != NULL))
			{
	 ...
	 displacement_string_end are meaningful.  */
		...
		if (i.base_reg && i.base_reg->reg_type.bitfield.instance == RegD && i.base_reg->reg_type.bitfield.word && i.index_reg == 0 && i.log2_scale_factor == 0 && i.seg[i.mem_operands] == 0 && !operand_type_check(i.types[this_operand], disp))
		{
...
elf_symbol_resolved_in_segment_p(symbolS *fr_symbol, offsetT fr_var)
{
	...
	if ((symbol_get_bfdsym(fr_symbol)->flags & BSF_GNU_INDIRECT_FUNCTION) != 0)
		return 0;
...
i386_macro_fusible_p(enum mf_cmp_kind mf_cmp, enum mf_jcc_kind mf_jcc)
{
	if (mf_cmp == mf_cmp_alu_cmp)
		return ((mf_jcc >= mf_jcc_jc && mf_jcc <= mf_jcc_jna) || mf_jcc == mf_jcc_jl || mf_jcc == mf_jcc_jle);
	if (mf_cmp == mf_cmp_incdec)
		return (mf_jcc == mf_jcc_je || mf_jcc == mf_jcc_jl || mf_jcc == mf_jcc_jle);
...
i386_next_non_empty_frag(fragS *fragP)
{
	...
	for (fragP = fragP->fr_next;
		 (fragP != NULL && fragP->fr_type == rs_fill && fragP->fr_fix == 0);
		 fragP = fragP->fr_next)
		;
	return fragP;
}
...
i386_next_fusible_jcc_frag(fragS *maybe_cmp_fragP, fragS *pad_fragP)
{
	...
	if (pad_fragP->fr_type == rs_machine_dependent && (TYPE_FROM_RELAX_STATE(pad_fragP->fr_subtype) == BRANCH_PADDING))
	{
		branch_fragP = i386_next_non_empty_frag(pad_fragP);
		if (branch_fragP->fr_type != rs_machine_dependent)
			return NULL;
		if (TYPE_FROM_RELAX_STATE(branch_fragP->fr_subtype) == COND_JUMP && i386_macro_fusible_p(maybe_cmp_fragP->tc_frag_data.mf_type,
...
i386_classify_machine_dependent_frag(fragS *fragP)
{
		 ...
		 next_fragP = next_fragP->fr_next)
	{
		...
		if (next_fragP->fr_type == rs_machine_dependent)
	       ...
	       */
		   ...
		   a fused jcc.  */
					next_fragP->fr_subtype = ENCODE_RELAX_STATE(BRANCH_PREFIX, 0);
					next_fragP->tc_frag_data.max_bytes = next_fragP->tc_frag_data.max_prefix_length;
	...
	{
		if (fragP->fr_type != rs_machine_dependent || (TYPE_FROM_RELAX_STATE(fragP->fr_subtype) != BRANCH_PREFIX))
			continue;
	 ...
	 COND_JUMP_PREFIX.  */
			 ...
			 next_fragP = next_fragP->fr_next)
		{
			...
			if (TYPE_FROM_RELAX_STATE(next_fragP->fr_subtype) == BRANCH_PREFIX)
			{
				...
				if (max_prefix_length >= MAX_FUSED_JCC_PADDING_SIZE)
				{
					...
					max_prefix_length += next_fragP->tc_frag_data.max_bytes;
			}
			else if ((TYPE_FROM_RELAX_STATE(next_fragP->fr_subtype) == BRANCH_PADDING) || (TYPE_FROM_RELAX_STATE(next_fragP->fr_subtype) == FUSED_JCC_PADDING))
			{
...
long i386_generic_table_relax_frag(segT segment, fragS *fragP, long stretch)
{
	if (TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == BRANCH_PADDING || TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == FUSED_JCC_PADDING)
	{
		...
		if (fragP->tc_frag_data.padding_address && (fragP->tc_frag_data.padding_address != fragP->fr_address || padding_size))
			abort();
	...
	else if (TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == BRANCH_PREFIX)
	{
		...
		if (!padding_fragP)
			...
			return (fragP->tc_frag_data.length - fragP->tc_frag_data.last_length);
		...
		if (!fragP->tc_frag_data.padding_address)
			fragP->tc_frag_data.padding_address = padding_fragP->fr_address - (fragP->fr_address - stretch);
		...
		for (next_fragP = fragP;
			 ...
			 next_fragP = next_fragP->fr_next)
			if (next_fragP->fr_type == rs_machine_dependent && (TYPE_FROM_RELAX_STATE(next_fragP->fr_subtype) == BRANCH_PREFIX))
			{
				if (left_size)
				{
		...
		padding_size = i386_branch_padding_size(padding_fragP, (fragP->fr_address + fragP->tc_frag_data.padding_address));
		...
		if (padding_size == last_size)
		{
			...
			padding_fragP->tc_frag_data.padding_address = (fragP->fr_address + padding_size + fragP->tc_frag_data.padding_address);
			return 0;
		}
...
{
	if (TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == BRANCH_PADDING || TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == BRANCH_PREFIX || TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == FUSED_JCC_PADDING)
	{
...
#if defined(OBJ_ELF) || defined(OBJ_MAYBE_ELF)
		|| (IS_ELF && !elf_symbol_resolved_in_segment_p(fragP->fr_symbol,
														fragP->fr_var))
#endif
#if defined(OBJ_COFF) && defined(TE_PE)
					 ...
					 fragS *fragP)
{
	...
	if (TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == BRANCH_PADDING || TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == FUSED_JCC_PADDING || TYPE_FROM_RELAX_STATE(fragP->fr_subtype) == BRANCH_PREFIX)
	{
	...
	/* If size if less then four we are sure that the operand fits,
     ...
     then -/+ 2GB.  */
	if (DISP_SIZE_FROM_RELAX_STATE(fragP->fr_subtype) == 4 && object_64bit && ((addressT)(displacement_from_opcode_start - extension + ((addressT)1 << 31)) > (((addressT)2 << 31) - 1)))
	{
		as_bad_where(fragP->fr_file, fragP->fr_line,
...
void md_apply_fix(fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
	...
	if (fixP->fx_addsy != NULL && (fixP->fx_r_type == BFD_RELOC_32_PCREL || fixP->fx_r_type == BFD_RELOC_64_PCREL || fixP->fx_r_type == BFD_RELOC_16_PCREL || fixP->fx_r_type == BFD_RELOC_8_PCREL) && !use_rela_relocations)
	{
	 ...
	 relocations); see more below.  */
		...
		if (IS_ELF)
		{
			...
			if ((sym_seg == seg || (symbol_section_p(fixP->fx_addsy) && sym_seg != absolute_section)) && !generic_force_reloc(fixP))
			{
...
static bfd_boolean check_register(const reg_entry *r)
{
	...
	if ((r->reg_type.bitfield.dword || (r->reg_type.bitfield.class_width == SReg && r->reg_num > 3) || r->reg_type.bitfield.class_width == RegCR || r->reg_type.bitfield.class_width == RegDR) && !cpu_arch_flags.bitfield.cpui386)
		return FALSE;

	if (r->reg_type.bitfield.class_width == RegTR && (flag_code == CODE_64BIT || !cpu_arch_flags.bitfield.cpui386 || cpu_arch_isa_flags.bitfield.cpui586 || cpu_arch_isa_flags.bitfield.cpui686))
		return FALSE;

	if (r->reg_type.bitfield.class_width == RegMMX && !cpu_arch_flags.bitfield.cpummx)
		return FALSE;

	if (!cpu_arch_flags.bitfield.cpuavx512f)
	{
		if (r->reg_type.bitfield.zmmword || r->reg_type.bitfield.class_width == RegMask)
			return FALSE;
	...
	if (r->reg_type.bitfield.class_width == RegBND && !cpu_arch_flags.bitfield.cpumpx)
		return FALSE;
	...
	if (r->reg_flags & RegVRex)
	{
		if (!cpu_arch_flags.bitfield.cpuavx512f || flag_code != CODE_64BIT)
			return FALSE;
	...
	if (((r->reg_flags & (RegRex64 | RegRex)) || r->reg_type.bitfield.qword) && (!cpu_arch_flags.bitfield.cpulm || r->reg_type.bitfield.class_width != RegCR) && flag_code != CODE_64BIT)
		return FALSE;

	if (r->reg_type.bitfield.class_width == SReg && r->reg_num == RegFlat && !intel_syntax)
		return FALSE;
...
parse_real_register(char *reg_string, char **end_op)
{
	...
	*end_op = s;
	...
	{
		if (!cpu_arch_flags.bitfield.cpu8087 && !cpu_arch_flags.bitfield.cpu287 && !cpu_arch_flags.bitfield.cpu387 && !allow_pseudo_reg)
			return (const reg_entry *)NULL;
...
#if (defined(OBJ_ELF) || defined(OBJ_MAYBE_ELF) || defined(TE_PE) || defined(TE_PEP) || defined(OBJ_MACH_O))
...
int md_parse_option(int c, const char *arg)
{
	...
	case OPTION_64:
	{
		...
		for (l = list; *l != NULL; l++)
			if (CONST_STRNEQ(*l, "elf64-x86-64") || strcmp(*l, "coff-x86-64") == 0 || strcmp(*l, "pe-x86-64") == 0 || strcmp(*l, "pei-x86-64") == 0 || strcmp(*l, "mach-o-x86-64") == 0)
			{
				default_arch = "x86_64";
				break;
			}
	...
	case OPTION_MBRANCHES_WITH_32B_BOUNDARIES:
		...
		align_branch = (align_branch_jcc_bit | align_branch_fused_bit | align_branch_jmp_bit);
		break;
...
void md_show_usage(FILE *stream)
{
  ...
  -s                      ignored\n"));
#endif
#if defined BFD64 && (defined(OBJ_ELF) || defined(OBJ_MAYBE_ELF) || defined(TE_PE) || defined(TE_PEP))
	fprintf(stream, _("\
...
{
	if (name[0] == GLOBAL_OFFSET_TABLE_NAME[0] && name[1] == GLOBAL_OFFSET_TABLE_NAME[1] && name[2] == GLOBAL_OFFSET_TABLE_NAME[2] && strcmp(name, GLOBAL_OFFSET_TABLE_NAME) == 0)
	{
		if (!GOT_symbol)
		{
			if (symbol_find(name))
...
void i386_cons_align(int ignore ATTRIBUTE_UNUSED)
{
	if (last_insn.kind != last_insn_directive && (bfd_section_flags(now_seg) & SEC_CODE))
	{
		last_insn.seg = now_seg;
...
void i386_validate_fix(fixS *fixp)
{
	...
	else
	{
	 ...
	 turn such PLT32 relocation into PC32 relocation.  */
		if (fixp->fx_addsy && (fixp->fx_r_type == BFD_RELOC_386_PLT32 || fixp->fx_r_type == BFD_RELOC_X86_64_PLT32) && symbol_section_p(fixp->fx_addsy))
			fixp->fx_r_type = BFD_RELOC_32_PCREL;
		if (!object_64bit)
		{
			if (fixp->fx_r_type == BFD_RELOC_386_GOT32 && fixp->fx_tcbit2)
...
tc_gen_reloc(asection *section ATTRIBUTE_UNUSED, fixS *fixp)
{
	...
	case BFD_RELOC_SIZE64:
		if (S_IS_DEFINED(fixp->fx_addsy) && !S_IS_EXTERNAL(fixp->fx_addsy))
		{
	...
	if ((code == BFD_RELOC_32 || code == BFD_RELOC_32_PCREL || code == BFD_RELOC_X86_64_32S) && GOT_symbol && fixp->fx_addsy == GOT_symbol)
	{
		if (!object_64bit)
	...
	if ((code == BFD_RELOC_64 || code == BFD_RELOC_64_PCREL) && GOT_symbol && fixp->fx_addsy == GOT_symbol)
	{
