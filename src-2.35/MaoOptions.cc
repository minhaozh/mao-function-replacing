...
void MaoOptions::TimerPrint() {
  ...
  for (OptionVector::iterator it1 = option_array_list->begin();
    ...
    total_secs += (*it1)->timer()->GetMillisecs();
  }
  ...
  for (OptionVector::iterator it = option_array_list->begin();
    ...
    if ((*it)->timer()->Triggered()) {
      fprintf(stderr, "  Pass: %-12s %5.1lf [millisec] %5.1lf%%\n",
	      (*it)->name(), (*it)->timer()->GetMillisecs(),
	      100.0 * (*it)->timer()->GetMillisecs() / total_secs);
    }
  }
  fprintf(stderr, "Total accounted for: %5.1lf [millisec]\n", total_secs );
}


// Simple (static) constructor to allow registration of
// option arrays.
// pass调用MaoOptionRegister来将自己的optionArray放入list
...
//
void MaoOptions::Reparse(MaoUnit *unit, MaoUnit *unit_mf, MaoPassManager *pass_man) {
  Parse(NULL, mao_options_, false, unit, unit_mf, pass_man);
}

// argv0是程序路径，arg是以--mao=开头的编译选项但是去掉了--mao=
void MaoOptions::Parse(const char *argv0,
                       const char *arg, bool collect,
                       MaoUnit *unit, MaoUnit *unit_mf, MaoPassManager *pass_man) {
  ...
  for (RegisteredStaticOptionPassMap::const_iterator pass =
    ...
    // pass->first是pass的名字
    // 找到pass对应的optionArray，最开始的一定都是空的，因为宏展开的时候就是空的
    MaoOptionArray *current_opts = FindOptionArray(pass->first);
    MAO_ASSERT(current_opts);
    // pass->second是pass对应的MaoOptionMap，pass初始化的时候都为空
    // 给MaoOptionMap中的option赋初值，并把current_opts中的的值应用到MaoOptionMap
    ...
    if (isascii(arg[0])) {
      ...
      if (pass_name.size() != 0) {
        ...
        if (pass_man) {
            ...
            if (unit_creator) {
                if (strcmp(pass_name.c_str(), "MergeFunction") == 0)
                    pass_man->LinkPass(unit_creator(options, unit_mf, unit));
                else
                    pass_man->LinkPass(unit_creator(options, unit, NULL));
                func_pass_man = NULL;
