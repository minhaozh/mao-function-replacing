...
{
	valueT number =
		((generic_bignum[1] & LITTLENUM_MASK) << LITTLENUM_NUMBER_OF_BITS) | (generic_bignum[0] & LITTLENUM_MASK);
	number &= 0xffffffff;
	return number;
}
...
#undef valuesize
	...
	if ((NUMBERS_WITH_SUFFIX || flag_m68k_mri) && suffix != NULL && input_line_pointer - 1 == suffix)
		c = *input_line_pointer++;
...
operand(expressionS *expressionP, enum expr_mode mode)
{
	...
	case '0':
		...
		case 'b':
			if (LOCAL_LABELS_FB && !flag_m68k_mri && input_line_pointer[1] != '0' && input_line_pointer[1] != '1')
			{
			...
			/* Fall through.  */
		case 'B':
			if (input_line_pointer[1] == '0' || input_line_pointer[1] == '1')
			{
				input_line_pointer++;
				integer_constant(2, expressionP);
				break;
		...
		case 'f':
		 ...
		 and try to deal with parsing the rest later.  */
				if (!is_end_of_line[(unsigned char)input_line_pointer[1]] && strchr(FLT_CHARS, 'f') != NULL)
				{
		     ...
		     expression?  */
					is_label = (cp == input_line_pointer + 1 || (cp == input_line_pointer + 2 && (cp[-1] == '-' || cp[-1] == '+')) || *cp == 'f' || *cp == 'b');
				}
				if (is_label)
				{
					input_line_pointer--;
	...
	case '(':
		...
		if ((c == '(' && *input_line_pointer != ')') || (c == '[' && *input_line_pointer != ']'))
		{
			if (*input_line_pointer)
	...
	unary:
		...
		if (expressionP->X_op == O_constant)
		{
			...
			if (c == '-')
			{
				expressionP->X_add_number = -(addressT)expressionP->X_add_number;
		...
		else if (expressionP->X_op == O_big && expressionP->X_add_number <= 0 && c == '-' && (generic_floating_point_number.sign == '+' || generic_floating_point_number.sign == 'P'))
		{
	...
	case '.':
		...
		else if ((strncasecmp(input_line_pointer, "startof.", 8) == 0 && !is_part_of_name(input_line_pointer[8])) || (strncasecmp(input_line_pointer, "sizeof.", 7) == 0 && !is_part_of_name(input_line_pointer[7])))
		{
			int start;

			start = (input_line_pointer[1] == 't' || input_line_pointer[1] == 'T');
	...
	de_fault:
		...
		isname:
	     ...
	     value now.  */
			...
			if (mode != expr_defer && segment == absolute_section && !S_FORCE_RELOC(symbolP, 0))
			{
				expressionP->X_op = O_constant;
...
void expr_set_rank(operatorT op, operator_rankT rank)
{
		  ...
		  enum expr_mode mode /* Controls behavior.  */)
{
	...
	while (op_left != O_illegal && op_rank[(int)op_left] > rank)
	{
		...
		know(op_right == O_illegal || op_left == O_index || op_rank[(int)op_right] <= op_rank[(int)op_left]);
		...
		if (mode == expr_defer && ((resultP->X_add_symbol != NULL && S_IS_FORWARD_REF(resultP->X_add_symbol)) || (right.X_add_symbol != NULL && S_IS_FORWARD_REF(right.X_add_symbol))))
			goto general;
...
#define md_register_arithmetic 1
#endif
			if (op_left == O_add && right.X_op == O_constant && (md_register_arithmetic || resultP->X_op != O_register))
		{
			/* X + constant.  */
			add_to_result(resultP, right.X_add_number, right.X_extrabit);
		}
		/* This case comes up in PIC code.  */
		else if (0 && op_left == O_subtract && right.X_op == O_symbol && resultP->X_op == O_symbol && retval == rightseg
#ifdef md_allow_local_subtract
				 && md_allow_local_subtract(resultP, &right, rightseg)
#endif
				 && ((SEG_NORMAL(rightseg) && !S_FORCE_RELOC(resultP->X_add_symbol, 0) && !S_FORCE_RELOC(right.X_add_symbol, 0)) || right.X_add_symbol == resultP->X_add_symbol) && frag_offset_fixed_p(symbol_get_frag(resultP->X_add_symbol), symbol_get_frag(right.X_add_symbol), &frag_off))
		{
			offsetT symval_diff = S_GET_VALUE(resultP->X_add_symbol) - S_GET_VALUE(right.X_add_symbol);
		...
		else if (op_left == O_subtract && right.X_op == O_constant && (md_register_arithmetic || resultP->X_op != O_register))
		{
			/* X - constant.  */
			subtract_from_result(resultP, right.X_add_number, right.X_extrabit);
		}
		else if (op_left == O_add && resultP->X_op == O_constant && (md_register_arithmetic || right.X_op != O_register))
		{
			/* Constant + X.  */
		...
		else if (resultP->X_op == O_constant && right.X_op == O_constant)
		{
			...
			if ((valueT)v >= sizeof(valueT) * CHAR_BIT && (op_left == O_left_shift || op_left == O_right_shift))
			{
		...
		else if (resultP->X_op == O_symbol && right.X_op == O_symbol && (op_left == O_add || op_left == O_subtract || (resultP->X_add_number == 0 && right.X_add_number == 0)))
		{
			/* Symbol OP symbol.  */
			...
			else if (op_left == O_subtract)
			{
				subtract_from_result(resultP, right.X_add_number,
									 right.X_extrabit);
				if (retval == rightseg && SEG_NORMAL(retval) && !S_FORCE_RELOC(resultP->X_add_symbol, 0) && !S_FORCE_RELOC(right.X_add_symbol, 0))
				{
...
int resolve_expression(expressionS *expressionP)
{
	...
	case O_logical_and:
	case O_logical_or:
		if (!snapshot_symbol(&add_symbol, &left, &seg_left, &frag_left) || !snapshot_symbol(&op_symbol, &right, &seg_right, &frag_right))
			return 0;
	 ...
	 the case of addition or subtraction of a constant above.  */
		frag_off = 0;
		if (!(seg_left == absolute_section && seg_right == absolute_section) && !(op == O_eq || op == O_ne) && !((op == O_subtract || op == O_lt || op == O_le || op == O_ge || op == O_gt) && seg_left == seg_right && (finalize_syms || frag_offset_fixed_p(frag_left, frag_right, &frag_off) || (op == O_gt && frag_gtoffset_p(left, frag_left, right, frag_right, &frag_off))) && (seg_left != reg_section || left == right) && (seg_left != undefined_section || add_symbol == op_symbol)))
		{
			if ((seg_left == absolute_section && left == 0) || (seg_right == absolute_section && right == 0))
			{
			...
			else if (op == O_multiply && seg_left == absolute_section && left == 1)
			{
				seg_left = seg_right;
				left = right;
				add_symbol = op_symbol;
			...
			else if ((op == O_multiply || op == O_divide) && seg_right == absolute_section && right == 1)
			{
				op = O_symbol;
				break;
			}
			else if (!(left == right && ((seg_left == reg_section && seg_right == reg_section) || (seg_left == undefined_section && seg_right == undefined_section && add_symbol == op_symbol))))
				return 0;
		...
		case O_eq:
		case O_ne:
			left = (left == right && seg_left == seg_right && (finalize_syms || frag_left == frag_right) && (seg_left != undefined_section || add_symbol == op_symbol)
						? ~(valueT)0
						: 0);
			if (op == O_ne)
				left = ~left;
...
char get_symbol_name(char **ilp_return)
{
	...
	*ilp_return = input_line_pointer;
	/* We accept FAKE_LABEL_CHAR in a name in case this is being called with a
     constructed string.  */
	if (is_name_beginner(c = *input_line_pointer++) || (input_from_string && c == FAKE_LABEL_CHAR))
	{
