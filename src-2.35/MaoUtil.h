...
private:
  ...
  void InitObj(int number_of_bits)
  {
    ...
    number_of_bits_ = number_of_bits;
    number_of_words_ = (number_of_bits - 1) / (sizeof(unsigned long long) * 8) + 1;
    word_ = new unsigned long long[number_of_words_];
    for (int i = 0; i < number_of_words_; i++)
      word_[i] = 0;
  }
  ...
  void VerifyBitString()
  {
    ...
    MAO_ASSERT(((number_of_words_ * bits_per_word) >= number_of_bits_) && (((number_of_words_ - 1) * bits_per_word) < number_of_bits_));
#endif
    if (!IsUndef())
    {
