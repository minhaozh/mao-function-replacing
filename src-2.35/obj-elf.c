...
obj_elf_section (int push)
{
  ...
  link_section(push, name, link_arguments,FALSE);
...
obj_elf_line (int ignore ATTRIBUTE_UNUSED)
{
  ...
  int lineNum=get_absolute_expression ();
  new_logical_line (NULL, lineNum);
  link_line_directive(lineNum);
  demand_empty_rest_of_line ();
}
  ...
  if (exp.X_op == O_constant)
    {
      ...
      link_size((char *)symbol_get_bfdsym(sym)->name, exp.X_add_number, 0);
  ...
  *input_line_pointer = c;
  ...
  if (type & BSF_FUNCTION)
      if (type & BSF_GNU_INDIRECT_FUNCTION)
        link_type(sym, INDIRECT_FUNCTION_SYMBOL, 0);
      else
        link_type(sym, FUNCTION_SYMBOL, 0);
  else if ((type & BSF_OBJECT) && (type & BSF_GNU_UNIQUE))
        link_type(sym, UNIQUE_OBJECT_SYMBOL, 0);
