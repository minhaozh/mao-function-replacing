...
/********************new added code*********************/
#include "irlink.h"
#include "tc-aarch64-helper.h"                              //This needs to be finished later
/********************new added code*********************/
...
//Note: According to what I know so far these days, there are no something like seg_prefix in aarch64 assembly.
//This remains to be seen and needs for me to look for more info.
...
static INLINE int
operand_type_check (arm_operand_type t, enum operand_type c)        //In ARM, is this ok here? In mao code, it's
{
  switch (c)
  {
  case reg:
...
static bfd_boolean
create_register_alias (char *newname, char *p)    //newname points to the start of alias, p points to the end of str
{
  const reg_entry *old;
  char *oldname, *nbuf;
  size_t nlen;
  ...
  oldname = p;
  if (strncmp (oldname, " .req ", 6) != 0)        //the inst is like "oldname .req "
    return FALSE;

  oldname += 6;                                   //oldname points to the start of old register-name
  if (*oldname == '\0')
    return FALSE;

  /********************new added code*********************/
  link_req_directive(newname, oldname);
  /********************new added code*********************/
...
s_unreq (int a ATTRIBUTE_UNUSED)
{
  ...
  while (*input_line_pointer != 0
         && *input_line_pointer != ' ' && *input_line_pointer != '\n')    //This is to locate the end of alias
    ++input_line_pointer;
...
s_bss (int ignore ATTRIBUTE_UNUSED)
{
  ...
  link_section(0, ".bss", arguments, TRUE);
  /********************new added code*********************/
...
s_ltorg (int ignored ATTRIBUTE_UNUSED)
{
  ...
  for (align = 2; align <= 4; align++)
  {
    ...
    char csize[1];
    csize[0] = size + '0';
    strcat(litname, &csize);
    strcat(litname, "\002");
    for (entry = 0; entry < pool->next_free_entry; entry++)
    {
      if (pool->literals[entry].exp.X_op == O_big)
      {
        char hexlitname[128] = "";
        LITTLENUM_TYPE generic_bignum2[SIZE_OF_LARGE_NUMBER + 6];
        memcpy(generic_bignum2, pool->literals[entry].bignum,
               CHARS_PER_LITTLENUM * pool->literals[entry].exp.X_add_number);
        for (int i = 0; i < pool->literals[entry].exp.X_add_number; i++)
        {
          char litn[8];
          sprintf(litn, "%x", generic_bignum2[i]);
          strcat(litn, hexlitname);
          memcpy(hexlitname, litn, strlen(litn));
        }
        strcat(litname, "0x");
        strcat(litname, hexlitname);
        strcat(litname, "\002");
      }
      if (pool->literals[entry].exp.X_op == O_constant)
      {
        char numbername[128];
        snprintf(numbername,sizeof(numbername), "%d",pool->literals[entry].exp.X_add_number);
        strcat(litname, numbername);
        strcat(litname, "\002");
      }
      if (pool->literals[entry].exp.X_op == O_symbol)
      {
        strcat(litname, S_GET_NAME(pool->literals[entry].exp.X_add_symbol));
        strcat(litname, "\002");
      }
    }
    sprintf(sym_name, "$$lit_\002%s%x", litname, pool->id);
...
static void
s_aarch64_elf_cons (int nbytes)       //The func handle long and word with nbytes = 4 and dword/xword with nbytes = 4
                                      //QUES: How to distinguish them? 
{
  expressionS exp;
  ...
  do
  {
    ...
    expression (&exp);                //exp is like .xword  "local+0xffffffffffffffff", which is 64-bit, should be stored in long int type

    if (exp.X_op != O_symbol)
    {
      link_dr_directive(&exp, nbytes);
      emit_expr (&exp, (unsigned int) nbytes);                //This func will modify the expr
    }
    else
    {
...
s_variant_pcs (int ignored ATTRIBUTE_UNUSED)
{
  ...
  /********************new added code*********************/
  link_variant_pcs_directive(sym);
  /********************new added code*********************/
...
s_aarch64_inst (int ignored ATTRIBUTE_UNUSED)
{
  ...
  /********************new added code*********************/
  //For now, I am not sure whether .inst expressions,[...] is true
  //if so, I need to use an array to store it.
  //Now I assume there is only one expr(Hehehe)
  //int i = 0;
  //expressionS * exp_array;
  /********************new added code*********************/
  ...
  do
  {
    ...
    emit_expr (&exp, 4);
    /********************new added code*********************/
    link_inst_directive(&exp);
    /********************new added code*********************/
  }
  while (*input_line_pointer++ == ',');           // .inst expression[,...]
...
{
  /********************new added code*********************/
  link_cfi_b_key_frame_directive();
  /********************new added code*********************/
...
s_tlsdesccall (int ignored ATTRIBUTE_UNUSED)
{
  ...
  /********************new added code*********************/
  link_tlsdesccall_directive(&exp);
  /********************new added code*********************/
...
const pseudo_typeS md_pseudo_table[] = {
  ...
  {"pool", s_ltorg, 0},
  {"cpu", s_aarch64_cpu, 0},
  {"arch", s_aarch64_arch, 0},                                                //This correspondings to set_cpu_arch() in tc-i386.c
  ...
  {"dc.a", s_aarch64_elf_cons, 8},
  {"octa", s_aarch64_elf_cons, 16},
  {0, 0, 0}
};

...
parse_half (char **str, int *internal_fixup_p)
{
  ...
  *internal_fixup_p = 0;
  ...
  if (! my_get_expression (&inst.reloc.exp, &p, GE_NO_PREFIX, 1))     //Here we get the expression
    return FALSE;
...
vector_reg_list:
    ...
    case AARCH64_OPND_HALF:
    {
      ...
      po_misc_or_fail (parse_half (&str, &internal_fixup_p));   //If we have lsl expr here, we will get the X_addnumber calculated
      if (internal_fixup_p)
        aarch64_set_gas_internal_fixup (&inst.reloc, info, 0);
      skip_whitespace (str);
      if (skip_past_comma (&str))       //This is for the explict relocation using LSL  
                                        //but the relocation in expr using << / >> is handled in parse_half()
      {
        /* {, LSL #<shift>}  */
        if (! aarch64_gas_internal_fixup_p ())
        {
      ...
      if (! process_movw_reloc_info ())           //Here we get the shiter_amount according to reloc.type and set it to the inst
        goto failure;

      /*********New added Code***********
      if(inst.reloc.exp.X_op == O_constant)       //By this we can save the LSL for symbol and calculate the imm right now
                                                  //However, we need to note that movz's imm should be 16 bit, and when the result of cal is out of range 
                                                  //It could be wrong. So if we need to recorrect it and output the result after calculating, change the movz or otherthings into mov
                                                  //But this is way to complicated, so I just leave it here and try to save the imm, LSL shifter format
                                                  //[TODO]Find out how the reloc_prefix of operand works
        inst.base.operands[i].shifter.amount = 0;
        inst.base.operands[i].opcode = 
      /*********New added Code************/
    }
    break;
...
md_assemble (char *str)
{
  ...
  /********************new added code*********************/
  char str_verbatim[1024];
  strncpy(str_verbatim, str, 1024);
  /********************new added code*********************/
  ...
  if (!template)
  {
    ...
    if (!create_register_alias (str, p))        //str is the new name, p points to the end of str
  ...
  do
  {
        ...
        && do_encode (inst_base->opcode, &inst.base, &inst_base->value))
    {
          ...
          || !inst.reloc.need_libopcodes_p)
      {
        output_inst (NULL);
        /********************new added code*********************/
        link_insn(&inst, sizeof(inst), CODE_64BIT, str_verbatim);              //Here, the flag code is not used because we dont have to consider the 16/32/64 bit code
        //so we can just set it as CODE64BIT to be convinient
        //TODO: whether this is ok to simplify it
        /********************new added code*********************/
      }
      else
      {
  ...
  output_operand_error_report (str, FALSE);                             //I dont think this needs to be commented
}
                        ...
                        bfd_boolean ext_only)
{
  ...
  *opt_p = ext_set;
  ...
  while (str != NULL && *str != 0)
  {
    ...
    if (!ext_only)                                        //has more than one extension?
    {
      if (*str != '+')
      {
      ...
      ext = strchr (++str, '+');                        //find the next extension string
    }

    if (ext != NULL)
      optlen = ext - str;
    else
      optlen = strlen (str);                            //when there is only one extension

    if (optlen >= 2 && strncmp (str, "no", 2) == 0)     //str: "noxxxx"
    {
      if (adding_value != 0)
        adding_value = 0;
      optlen -= 2;
...
/*
//This was specially for func s_aarch64_arch_extension()   Or maybe other functions in future?
//But now I dont think this is neccessary
static int
aarch64_parse_features (const char *str, const aarch64_feature_set **opt_p,
                        bfd_boolean ext_only, struct MaoStringPiece & e)
{
  int adding_value = -1;
  aarch64_feature_set *ext_set = XNEW (aarch64_feature_set);
  ...
  *opt_p = ext_set;
  ...
  while (str != NULL && *str != 0)
  {
    ...
    if (!ext_only)            //has more than one extension? 
                              //s_aarch64_arch_extension() call this func with ext_only = True
    {
      if (*str != '+')
      {
        as_bad (_("invalid architectural extension"));
      ...
      ext = strchr (++str, '+');                        //find start of the next extension string
    }
    ...
    for (opt = aarch64_features; opt->name != NULL; opt++)
      if (strncmp (opt->name, str, optlen) == 0)
      {
        ...
        if (adding_value)
        {
          ...
          AARCH64_MERGE_FEATURE_SETS (*ext_set, *ext_set, set);
          /********************new added code********************
          e = struct MaoStringPiece({str,(int)optlen}); 
          /********************new added code********************
        }
        else
        {
...
s_aarch64_cpu (int ignored ATTRIBUTE_UNUSED)                        
{
  ...
  *input_line_pointer = 0;
  ...
  /********************new added code*********************/
  struct MaoStringPiece description = {name,strlen(name)};
  link_cpu_directive(description);
  /********************new added code*********************/
  ...
  for (opt = aarch64_cpus + 1; opt->name != NULL; opt++)
    ...
    {
      mcpu_cpu_opt = &opt->value;
      if (ext != NULL)                                                  //ext is the descriotion end
...
s_aarch64_arch (int ignored ATTRIBUTE_UNUSED)
{
  ...
  while (*input_line_pointer && !ISSPACE (*input_line_pointer))       //This works like SKIP_WHITESPACE() and get to the symbol end
    input_line_pointer++;
  saved_char = *input_line_pointer;
  *input_line_pointer = 0;

  ext = strchr (name, '+');                                           //Finding index of '+' in current assembly line

  if (ext != NULL)
    optlen = ext - name;                                              //optlen means characters between beginner of symbol and +
  else
    optlen = strlen (name);                                           //It means the whole name string is the arch string

  /* Skip the first "all" entry.  */
  for (opt = aarch64_archs + 1; opt->name != NULL; opt++)             //aarch64_archs is the array of all type string of arm arch
  {
    if (strlen (opt->name) == optlen
        && strncmp (name, opt->name, optlen) == 0)
    {
      ...
      //name :"xxxxx\0" so the len of it contains the len of arch_name and features
      /********************new added code*********************/

      mcpu_cpu_opt = &opt->value;
      if (ext != NULL)
        if (!aarch64_parse_features (ext, &mcpu_cpu_opt, FALSE))
          //aarch64_parse_features will handle the features(more than one)
          return;                                                     //Only when it goes error, it weill return
...
s_aarch64_arch_extension (int ignored ATTRIBUTE_UNUSED)
{
  ...
  while (*input_line_pointer && !ISSPACE (*input_line_pointer))
    input_line_pointer++;
  saved_char = *input_line_pointer;             //input_line_pointer is the end of line
  *input_line_pointer = 0;

  /********************new added code*********************/
  struct MaoStringPiece description = {name, strlen(name)};
  link_arch_extension_directive(description);
  /********************new added code*********************/
