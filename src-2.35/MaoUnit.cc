...
void MaoUnit::ReplaceLastEntry(MaoEntry* e)
{
  EntryID entry_index = entry_vector_.size();
  e->set_id(entry_index);
  auto iter = sub_sections_.end()-1;
  SubSection *ss = *iter;
  ss->replace_last_entry(e);
  entry_vector_[entry_index-1]=e;
}
MaoEntry* MaoUnit::GetLastEntry()
{
  auto iter = sub_sections_.end()-1;
  SubSection *ss = *iter;
  return ss->last_entry();
...
std::vector<SubSection *> MaoUnit::GetSubSectionsVector() {
...
// 如果段中有上一份子段，连接两个子段中最后一个和第一个的先后关系
...
InstructionEntry *MaoUnit::CreateInstruction(aarch64_instruction *instruction,
                                             Function *function) {
  enum flag_code flag;
  switch (arch_) {
    case X86_64:
...
InstructionEntry *MaoUnit::CreateInstruction(const char *opcode_str,
                                             unsigned int base_opcode,
                                             Function *function) {
//  aarch64_instruction insn;
//  memset(&insn, 0, sizeof(aarch64_instruction));
...
//
//  aarch64_instruction *insn = e->instruction();
//  insn->operands = 2;
//  insn->reg_operands = 2;
//
//  insn->suffix = WORD_MNEM_SUFFIX;
//
//  insn->types[0].bitfield.instance = 1;
//  insn->types[1].bitfield.instance = 1;
//
...
//
//  aarch64_instruction *in_insn = e->instruction();
//  in_insn->operands = 1;
...
LabelEntry *MaoUnit::CreateLabel(const char *labelname,
  ...
  LabelEntry *l = new LabelEntry(labelname, 0, NULL, this, 0);
...
bool MaoUnit::AddEntry(MaoEntry *entry,
                       bool  create_default_section) {          //This includes handling the directive
...
// 判断是否需要创建subsection
  ...
  // 如果current_subsection_为空create_default_section为false
  if (!current_subsection_ &&
    ...
    if (de == NULL || (de->op() != DirectiveEntry::SECTION
                       && de->op() != DirectiveEntry::PUSHSECTION
                       && de->op() != DirectiveEntry::POPSECTION
                       && de->op() != DirectiveEntry::TEXT_SECTION
                       && de->op() != DirectiveEntry::DATA_SECTION
                       && de->op() != DirectiveEntry::BSS_SECTION)) {                  //如果不是directive或者不是.section的其他directive
...
// 根据entry类型分类
  ...
  switch (entry->Type()) {
    ...
    case MaoEntry::LABEL:
      ...
      // 创建<labelname,labelentry>添加到labels_ map中
    ...
    case MaoEntry::DIRECTIVE:
      ...
      switch (directive_entry->op()) {
        case DirectiveEntry::PUSHSECTION:
        case DirectiveEntry::POPSECTION:
        case DirectiveEntry::SECTION:
          {
            ...
            current_subsection_ = AddNewSubSection(s_section_name->c_str(), 0,
                                                   entry);
            if(directive_entry->op()==DirectiveEntry::POPSECTION)
            {
                DirectiveEntry::OperandVector  v;
                directive_entry->setOperands(v);
            }
            break;
          }
        case DirectiveEntry::TEXT_SECTION:
        {

          std::string section_name=".text";
          std::string *s_section_name = &section_name;
          prev_subsection_ = current_subsection_;
          // 在这个section中添加一个新子段
          current_subsection_ = AddNewSubSection(s_section_name->c_str(), 0,
                                                 entry);
        ...
        case DirectiveEntry::DATA_SECTION:
        {
        ...
        case DirectiveEntry::BSS_SECTION:
        {
        ...
        case DirectiveEntry::SET:
          {
            ...
            if (op_expr->type == DirectiveEntry::EXPRESSION) {
              ...
              if (op_as_expr->X_op == O_symbol) {
                  // FIXME(@getianao): special case for dot symbol
                if(strcmp((char *)symbol_get_bfdsym(op_as_expr->X_add_symbol)->name,
                          FAKE_LABEL_NAME) != 0)
                {
                      Symbol *op_symbol_2 =
  ...
  // 此外，还需要将entry放到一个包括所有entry的vector里
  // Add the entry to the compilation unit
  entry_vector_.push_back(entry);

// 更新subsection的last_entry和entry对应的subsection的table里
...
//  TODO(@getianao): Skip the entry which didn't define. Check it later.
    if(entry == NULL)
      continue;
...
//  MAO_ASSERT(current_subsection_);
//FIXME(@getianao):Here exists bug when current_subsection_ is null
  if(current_subsection_!=NULL)
  {
...
void MaoUnit::PopSubSection(int line_number) {
  ...
  operands.push_back(new DirectiveEntry::Operand(
                         ss->name().c_str()));
  DirectiveEntry *directive =
      new DirectiveEntry(DirectiveEntry::POPSECTION, operands,
