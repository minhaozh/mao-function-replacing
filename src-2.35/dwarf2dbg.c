...
dwarf2_directive_filename (void)
{
  ...
  if (DWARF2_LINE_VERSION > 4)
      ...
      if (strncmp (input_line_pointer, "md5", 3) == 0)
	{
	  ...
	  expressionS exp;
      char *md5End=strchr(input_line_pointer,'\n');
      md5Str=(char*)malloc(sizeof(char) * (md5End-input_line_pointer));
      memcpy(md5Str,input_line_pointer,md5End-input_line_pointer);
  ...
  if(with_md5)
  {
      link_file_md5_directive(dirname, filename, &filenum, md5Str);
      free(md5Str);
  }
  else
...
dwarf2_directive_loc (int dummy ATTRIBUTE_UNUSED)
{
  ...
  while (ISALPHA (*input_line_pointer))
      ...
      else if (strcmp (p, "discriminator") == 0)
	{
	  ...
	  options[num_options++] = GetStringPiece("discriminator");
	  struct MaoStringPiece  sp_value =  {helper_pointer,
                                            input_line_pointer-helper_pointer};
	  options[num_options++] =  sp_value;
	}
      else if (strcmp (p, "view") == 0)
	{
	  ...
	  if (ISDIGIT (*input_line_pointer)
	      ...
	      else
		{
	      ...
	      options[num_options++] = GetStringPiece("view");
	      struct MaoStringPiece  sp_value =  {helper_pointer,
                                               input_line_pointer-helper_pointer};
	      options[num_options++] =  sp_value;
	    }
	      ...
	      if (S_IS_DEFINED (sym) || symbol_equated_p (sym))
		{
	      ...
	      options[num_options++] = GetStringPiece("view");
	      struct MaoStringPiece  sp_value =  {name,
                                                strlen(name)};
	      options[num_options++] =  sp_value;
	    }
	  current.view = sym;
